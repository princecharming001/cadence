...
async function harvestMetaAds(userId, terms) {
  ...
  const out = await generateJson({
    system: `You analyze ad creative to find the AD FORMAT that's working — the structural template (hook, problem, proof, CTA shape), not the specific product. Return: name (short label), pattern (a reusable template), why_it_works, suggested_hook (an ad opener in this format).`,
    user: corpus,
    schema: { type: 'object', required: ['name', 'pattern'], properties: { name: { type: 'string' }, pattern: { type: 'string' }, why_it_works: { type: 'string' }, suggested_hook: { type: 'string' } } },
    maxTokens: 600, toolName: 'emit_ad',
  }).catch(() => null)
