...
// TEXT trends (X / LinkedIn). No new API spend: distills the winning FORMAT from
// the niche corpus we already keep — X inspiration_posts (top posts from the
// accounts they study) and the LinkedIn mentor corpus — into a reusable pattern
// that trendingBlock() then injects into every post we write for that platform.
// Bounded: skips if we distilled a fresh text format for this platform recently.
async function harvestText(userId, platform, { onStep } = {}) {
  const since = new Date(Date.now() - 3 * 864e5).toISOString()
  const { count: recent } = await admin.from('trend_formats').select('id', { count: 'exact', head: true })
    .eq('user_id', userId).eq('platform', platform).eq('source', 'harvest').eq('kind', 'format').gte('created_at', since)
  if (recent) return { skipped: 'fresh' }
  const corpus = (await inspirationCorpus(userId, platform, 10).catch(() => [])).filter(p => String(p.text || '').length > 40)
  if (corpus.length < 3) return { skipped: 'thin corpus' }
  onStep?.(`Distilling what's working on ${platform === 'linkedin' ? 'LinkedIn' : 'X'}…`)
  const f = await analyzeViralText({ posts: corpus.slice(0, 8), platform }, { userId, save: true }).catch(() => null)
  if (f?.id) { await admin.from('trend_formats').update({ source: 'harvest' }).eq('id', f.id); return { saved: 1 } }
  return { saved: 0 }
}
...
export async function runTrendHarvest(userId, { platforms = ['tiktok', 'instagram'], deepN = 3, onStep } = {}) {
  ...
  // Text platforms: distill the niche's winning format from corpora we already
  // keep (no extra reads). Runs for whichever text platforms were requested.
  for (const platform of platforms.filter(p => p === 'x' || p === 'linkedin')) {
    onStep?.(`Scanning ${platform === 'linkedin' ? 'LinkedIn' : 'X'} for what's working…`)
    try {
      const r = await harvestText(userId, platform, { onStep })
      if (r.saved) { summary.formats += r.saved; summary.byPlatform[platform] = { analyzed: r.saved } }
      else if (r.skipped) summary.notes.push(`${platform}: ${r.skipped}`)
    } catch (e) { summary.notes.push(`${platform}: ${String(e.message).slice(0, 60)}`) }
  }
...
export async function harvestDueTrends({ limit = 3, deepN = 2 } = {}) {
  // Video/ad harvest needs APIFY_TOKEN, but X/LinkedIn TEXT harvest does not —
  // so the sweep runs regardless; runTrendHarvest no-ops the parts it can't do.
  const stale = new Date(Date.now() - 22 * 3600 * 1000).toISOString()
  const platforms = APIFY_TOKEN ? ['tiktok', 'instagram', 'x', 'linkedin'] : ['x', 'linkedin']
  ...
  for (const p of due || []) {
    ...
    try { await runTrendHarvest(p.user_id, { platforms, deepN }); ran++ } catch {}
  }
  return { ran, platforms }
}
