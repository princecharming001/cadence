...
export async function attachRenderedVideos({ limit = 30 } = {}) {
  const { data: pending } = await admin.from('posts')
    .select('id, video_job_id, feeder_agent_id, created_at')
  ...
  for (const p of pending) {
    ...
    if (job.status === 'done' && job.video_url) {
      ...
      attached++
    } else if (job.status === 'failed' || job.status === 'needs_provider') {
      await admin.from('posts').update({ status: 'failed', error: `video render ${job.status}` }).eq('id', p.id)
