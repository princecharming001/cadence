...
async function persistFromUrl(url, job) {
  ...
  return persistBuffer(Buffer.from(await r.arrayBuffer()), job)
}
...
async function ttsWav(text, job) {
  ...
  try {
    const r = await fetch('https://api.openai.com/v1/audio/speech', {
      ...
      body: JSON.stringify({ model: 'gpt-4o-mini-tts', voice: 'alloy', input: String(text || '').slice(0, 1500), response_format: 'wav' }),
    })
    if (!r.ok) return null
...
export async function processVideoJob(job) {
  try {
    if (job.mode === 'edit') {
      // Always-works path: montage the user's media + any external media.
      ...
      if (!items.length) { await admin.from('video_jobs').update({ status: 'failed', error: 'No media to edit — attach Library media or paste a link.', status_detail: null }).eq('id', job.id); return { error: 'no media' } }
      await setDetail(job.id, `Editing ${items.length} clip${items.length > 1 ? 's' : ''} together…`)
      const { buffer, count } = await buildMontage({ items, aspect: job.aspect || 'vertical' })
      await setDetail(job.id, 'Uploading your video…')
      const hosted = await persistBuffer(buffer, job)
      await admin.from('video_jobs').update({ status: 'done', status_detail: `Montage of ${count} clip${count > 1 ? 's' : ''}`, video_url: hosted, provider: 'ffmpeg', error: null }).eq('id', job.id)
      return { done: true }
    }
