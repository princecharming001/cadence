...
import { renderTweetCard, renderRedditCard } from './clip-cards'
...
// Five offered styles, each an intrinsically DIFFERENT format (not caption
// variants) — modeled on what actually dominates Reels/TikTok for brand and
// faceless pages:
//   captions — spoken-word karaoke emphasis (the baseline everyone expects)
//   sludge   — gameplay second-screen, the retention anchor
//   tweet    — the clip framed as a viral tweet screenshot (social proof)
//   thread   — a 2-3 tweet mini-thread that ADVANCES while the clip plays
//   reddit   — the r/ story-time card opener, the #1 faceless format
// Tweet/reddit cards are REAL renders (lib/clip-cards.js, Satori + Inter) — a
// drawbox approximation never reads as a screenshot.
export const EDIT_FORMATS = [
  ...
  { key: 'cold_open', label: 'Cold open', desc: 'Opens on a bold full-screen hook card, then cuts to your clip with captions' },
  { key: 'sludge', label: 'Sludge split', desc: 'Your clip on top, gameplay underneath, captions at the seam' },
  { key: 'tweet', label: 'Tweet quote', desc: 'A real viral-tweet screenshot pinned over the clip, in your name' },
  { key: 'thread', label: 'Thread', desc: 'A 2-3 tweet mini-thread that advances as the clip plays' },
  { key: 'reddit', label: 'Reddit story', desc: 'r/ story-time card opens the clip, captions carry it from there' },
  // legacy keys (older jobs) — still render, no longer offered in the UI
  { key: 'hook', legacy: true }, { key: 'emphasis', legacy: true }, { key: 'cinematic', legacy: true },
  { key: 'brand', legacy: true }, { key: 'clean', legacy: true },
  { key: 'meme_bar', legacy: true }, { key: 'banner', legacy: true }, { key: 'progress', legacy: true },
]
// Formats whose text renders through the ASS/libass layer. cold_open is captioned
// like 'captions'; its hook card is prepended after the body renders.
const ASS_EDITS = ['captions', 'cold_open', 'sludge', 'hook', 'emphasis', 'cinematic', 'brand', 'tweet', 'thread', 'reddit']
// Formats that overlay a rendered social-card PNG.
const CARD_EDITS = ['tweet', 'thread', 'reddit']
...
// Overlay chains around the ASS subtitle layer: `pre` draws boxes/cards UNDER
// the text (tweet card, letterbox bars, endcard scrim); `post` draws text that
// libass doesn't own (legacy drawtext titles, the corner watermark).
function editOverlays(edit, { titleFile, watermark, durSec }) {
  if (!FONT) return { pre: '', post: '' }
  ...
  switch (edit) {
    case 'cinematic':
      // Documentary letterbox. Captions (italic, tracked-out) sit above the
      // bottom bar via the ASS layer. Deliberately no watermark — premium look.
      return { pre: `,drawbox=x=0:y=0:w=iw:h=170:color=black:t=fill,drawbox=x=0:y=ih-170:w=iw:h=170:color=black:t=fill`, post: '' }
    case 'brand':
      // Dark scrim for the final FOLLOW endcard (text via ASS). The pinned
      // handle pill is ASS too, so no corner watermark either.
      return { pre: durSec > 4 ? `,drawbox=x=0:y=0:w=iw:h=ih:color=black@0.82:t=fill:enable='gt(t,${(durSec - 1.8).toFixed(2)})'` : '', post: '' }
    case 'meme_bar':
      return { pre: '', post: `,drawbox=x=0:y=0:w=iw:h=250:color=white:t=fill` +
        (titleFile ? `,drawtext=${f}:textfile=${titleFile}:fontsize=52:fontcolor=black:line_spacing=10:x=(w-text_w)/2:y=(250-text_h)/2` : '') + wm }
    case 'banner':
      return { pre: '', post: `,drawbox=x=0:y=h-190:w=iw:h=120:color=black@0.62:t=fill` +
    ...
    case 'progress':
      return { pre: '', post: `,drawbox=x=0:y=h-12:w='max(1\\,iw*t/${Math.max(durSec, 1)})':h=12:color=white@0.85:t=fill` + wm }
    default: // captions / emphasis / hook / sludge / clean — text lives in ASS; just the watermark
      return { pre: '', post: wm }
  }
}

// On-video copy per clip (one Claude call): a hook title always; when a card
// format is in play, also platform-NATIVE copy — a viral-voice tweet, a 2-3
// part thread, and an r/ story-time title. With a transcript we write from
// what's actually said; without, we riff on the source.
async function writeClipCopy(picks, { sourceName, segments, wantCards }) {
  ...
  const cardSpec = wantCards ? `
- "tweet": the clip's idea written as a VIRAL TWEET (<=200 chars) — lowercase-casual X voice, one sharp thought, no hashtags, no emojis. It must read like a real banger screenshot, not a caption.
- "thread": 2-3 short tweets (<=160 chars each) that ADVANCE one idea across the clip — hook, development, payoff. Each stands alone.
- "reddit_title": the idea reframed as an r/ story-time post title (<=110 chars) — first person, specific, slightly unhinged curiosity ("My cofounder quit the day we hit $1M ARR. He was right.").
- "subreddit": one plausible niche subreddit name for it (no "r/" prefix, e.g. Entrepreneur, cscareerquestions, antiwork).` : ''
  try {
    const res = await anthropic.messages.create({
      model: 'claude-sonnet-4-6', max_tokens: wantCards ? 1400 : 500,
      system: `You write the on-video copy for short-form clips (Reels/TikTok). For each clip provide:
- "title": a scroll-stopping headline, <=9 words, casual, human, no quotes/hashtags/emojis.${cardSpec}
${segments?.length ? 'Base everything on what is actually said in that clip.' : `The clips are from "${sourceName || 'a video'}" — write hooks that fit any moment from it.`}
Respond ONLY JSON: {"clips":[{"title":"...",${wantCards ? '"tweet":"...","thread":["...","..."],"reddit_title":"...","subreddit":"..."' : ''}}]}`,
      messages: [{ role: 'user', content: `Write the copy for ${picks.length} clips:\n${ctx}` }],
    })
...
export async function transcribe(file, dir) {
...
const EMPHASIS_COLORS = ['&H0000FFFF&', '&H007BF03C&', '&H00395CFF&'] // yellow, green, orange-red

function buildKaraokeAss({ words, clipStart, clipEnd, alignment = 2, marginV = 480, fontSize = 92, title = null, variant = 'karaoke', brandHandle = null }) {
  const dur = clipEnd - clipStart
  const inRange = (words || []).filter(w => w.start >= clipStart - 0.2 && w.end <= clipEnd + 0.2)
  if (!inRange.length && !title && brandHandle == null) return null
  const clean = s => String(s || '').replace(/[{}\\]/g, '')
  // Lines track the speech rhythm: emphasis = 1-2 word power lines, the rest
  // <=3 words; break early on gaps so text lands with the voice.
  const perLine = variant === 'emphasis' ? 2 : variant === 'cinematic' ? 4 : 3
  const lines = [[]]
  for (const w of inRange) {
    const cur = lines[lines.length - 1]
    const gap = cur.length ? w.start - cur[cur.length - 1].end : 0
    if (cur.length >= perLine || (cur.length && gap > 0.8)) lines.push([w])
    else cur.push(w)
  }
  const events = lines.filter(l => l.length).map((l, li) => {
    const t0 = l[0].start - clipStart, t1 = Math.min(l[l.length - 1].end - clipStart + 0.12, dur)
    if (variant === 'cinematic') {
      // No karaoke sweep — quiet fades, sentence case, documentary subtitle feel.
      const body = l.map(w => clean(w.text)).join(' ')
      return `Dialogue: 0,${assTime(Math.max(0, t0))},${assTime(t1)},Cap,,0,0,0,,{\\fad(140,140)}${body}`
    }
    const kbody = l.map(w => `{\\k${Math.max(8, Math.round((w.end - w.start) * 100))}}${clean(w.text).toUpperCase()} `).join('').trim()
    if (variant === 'emphasis') {
      // Each line pops in with a scale punch and sweeps to its own color.
      const color = EMPHASIS_COLORS[li % EMPHASIS_COLORS.length]
      return `Dialogue: 0,${assTime(Math.max(0, t0))},${assTime(t1)},Cap,,0,0,0,,{\\fad(60,0)\\t(0,110,\\fscx106\\fscy106)\\1c${color}}${kbody}`
    }
    return `Dialogue: 0,${assTime(Math.max(0, t0))},${assTime(t1)},Cap,,0,0,0,,${kbody}`
  })
  ...
  if (title) {
    ...
    events.unshift(`Dialogue: 1,${assTime(0)},${assTime(Math.min(3.5, dur))},Hook,,0,0,0,,${wrapped}`)
  }
  // Brand: pinned handle pill all clip, then a big FOLLOW endcard over the scrim.
  if (brandHandle != null) {
    const h = clean(String(brandHandle).replace(/^@/, ''))
    if (h) events.push(`Dialogue: 1,${assTime(0)},${assTime(Math.max(0.5, dur - 1.8))},Pill,,0,0,0,,@${h}`)
    if (dur > 4) {
      events.push(`Dialogue: 2,${assTime(dur - 1.7)},${assTime(dur)},FollowBig,,0,0,0,,{\\fad(150,0)}FOLLOW`)
      events.push(`Dialogue: 2,${assTime(dur - 1.55)},${assTime(dur)},FollowSub,,0,0,0,,{\\fad(180,0)}${h ? `@${h}` : 'for more'}`)
    }
  }
  if (!events.length) return null
  // Caption style varies by variant; the card/pill/endcard styles are constant.
  const capStyle = variant === 'emphasis'
    ? `Style: Cap,Anton,116,&H0000FFFF,&H00FFFFFF,&H00000000,&H7F000000,-1,0,0,0,100,100,2,0,1,8,3,5,60,60,0,1`
    : variant === 'cinematic'
      ? `Style: Cap,Anton,54,&H00FFFFFF,&H00FFFFFF,&H00000000,&H00000000,0,-1,0,0,100,100,3,0,1,2,0,2,80,80,230,1`
...
Style: Pill,Anton,34,&H00FFFFFF,&H00FFFFFF,&H78101010,&H78101010,-1,0,0,0,100,100,1,0,3,14,0,1,54,54,84,1
Style: FollowBig,Anton,130,&H00FFFFFF,&H00FFFFFF,&H00000000,&H00000000,-1,0,0,0,100,100,4,0,1,0,0,8,60,60,800,1
Style: FollowSub,Anton,52,&H0000FFFF,&H0000FFFF,&H00000000,&H00000000,-1,0,0,0,100,100,2,0,1,0,0,8,60,60,968,1
...
// ── Brand outro ───────────────────────────────────────────────────────────────
// A 1.6s endcard appended to every clip when the job asks: dark frame, the
// company logo (or the handle as text), and a chime (assets/sounds/outro.mp3,
// operator-replaceable). The outro is sugar — any failure returns the original
// clip rather than failing the job.
const OUTRO_SOUND = path.join(process.cwd(), 'assets', 'sounds', 'outro.mp3')
async function appendOutro(outFile, dir, i, { logoUrl, handle }) {
  try {
    const meta = await probe(outFile)
    const W = meta.width, H = meta.height, dur = 1.6
    // Logo: fetched once per job into the temp dir.
    const logoFile = path.join(dir, 'outro-logo.png')
    if (logoUrl && !existsSync(logoFile)) {
      const res = await fetch(logoUrl, { signal: AbortSignal.timeout(10000) }).catch(() => null)
      if (res?.ok) await writeFile(logoFile, Buffer.from(await res.arrayBuffer()))
    }
    const haveLogo = existsSync(logoFile)
    const haveSound = existsSync(OUTRO_SOUND)
    const handleTxt = String(handle || '').replace(/^@/, '').trim()

    // 1) Render the outro segment.
    const seg = path.join(dir, `outro${i}.mp4`)
    const args = ['-f', 'lavfi', '-i', `color=c=0x0B0B0E:s=${W}x${H}:d=${dur}:r=30`]
    if (haveLogo) args.push('-i', logoFile)
    if (haveSound) args.push('-i', OUTRO_SOUND)
    else args.push('-f', 'lavfi', '-i', 'anullsrc=r=44100:cl=stereo')
    const wmTxt = FONT && handleTxt ? `,drawtext=fontfile=${FONT}:text='@${escText(handleTxt)}':fontsize=${Math.round(H * (haveLogo ? 0.024 : 0.05))}:fontcolor=white@0.9:x=(w-text_w)/2:y=${haveLogo ? 'h*0.62' : '(h-text_h)/2'}` : ''
    const vf = haveLogo
      ? `[1:v]scale=${Math.round(W * 0.38)}:-1[lg];[0:v][lg]overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2-${Math.round(H * 0.05)},fade=t=in:d=0.3${wmTxt}`
      : `[0:v]fade=t=in:d=0.3${wmTxt}`
    const aIdx = haveLogo ? 2 : 1
    args.push('-filter_complex', `${vf}[vo]`, '-map', '[vo]', '-map', `${aIdx}:a`, '-t', String(dur), ...['-c:v', 'libx264', '-preset', 'fast', '-crf', '21', '-c:a', 'aac', '-b:a', '128k'], '-y', seg)
    await run('ffmpeg', args, { timeoutMs: 120000 })

    // 2) Concat clip + outro (normalized fps/format/audio so concat is legal).
    const merged = path.join(dir, `final${i}.mp4`)
    if (meta.hasAudio) {
      await run('ffmpeg', ['-i', outFile, '-i', seg, '-filter_complex',
        ...
        '-map', '[v]', '-map', '[a]', '-c:v', 'libx264', '-preset', 'fast', '-crf', '21', '-c:a', 'aac', '-b:a', '128k', '-movflags', '+faststart', '-y', merged], { timeoutMs: 4 * 60 * 1000 })
    } else {
      await run('ffmpeg', ['-i', outFile, '-i', seg, '-filter_complex',
        `[0:v]fps=30,format=yuv420p,setsar=1[v0];[1:v]fps=30,format=yuv420p,setsar=1[v1];[v0][v1]concat=n=2:v=1[v]`,
        '-map', '[v]', '-c:v', 'libx264', '-preset', 'fast', '-crf', '21', '-movflags', '+faststart', '-y', merged], { timeoutMs: 4 * 60 * 1000 })
    }
    return merged
  } catch { return outFile }
}

// ── Cold open ─────────────────────────────────────────────────────────────────
// The single most common viral reel opener: ~1.5s of a bold full-screen hook
// statement ("POV: ...", a claim, a question) before the clip cuts in. Renders
// the hook on a dark card via the ASS layer (libass handles wrapping/centering),
// then concats it in front of the body — same normalize+concat as appendOutro.
async function prependColdOpen(bodyFile, dir, i, { hook, durHint }) {
  try {
    if (!hook || !FONT) return bodyFile
    const meta = await probe(bodyFile)
    const W = meta.width, H = meta.height, dur = 1.6
    const text = wrapTitle(hook, 16).split('\n').map(s => s.replace(/[{}\\]/g, '')).join('\\N')
...
[V4+ Styles]
Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
Style: Open,Anton,${Math.round(W * 0.11)},&H00FFFFFF,&H00FFFFFF,&H00000000,&H00000000,-1,0,0,0,100,100,2,0,1,5,0,5,90,90,90,1

[Events]
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text
Dialogue: 0,0:00:00.00,0:00:0${dur.toFixed(2)},Open,,0,0,0,,${text}
`
    const assFile = path.join(dir, `coldopen${i}.ass`)
    await writeFile(assFile, ass)
    const card = path.join(dir, `coldopen${i}.mp4`)
    await run('ffmpeg', ['-f', 'lavfi', '-i', `color=c=0x0B0B0E:s=${W}x${H}:d=${dur}:r=30`, '-f', 'lavfi', '-i', 'anullsrc=r=44100:cl=stereo',
      '-vf', `subtitles=filename='${assFile}':fontsdir='${FONTS_DIR}',fade=t=in:d=0.2`, '-t', String(dur),
      '-c:v', 'libx264', '-preset', 'fast', '-crf', '21', '-c:a', 'aac', '-b:a', '128k', '-y', card], { timeoutMs: 120000 })

    const merged = path.join(dir, `co-final${i}.mp4`)
    const va = meta.hasAudio
      ? `[0:v]fps=30,format=yuv420p,setsar=1[v0];[1:v]fps=30,format=yuv420p,setsar=1[v1];[0:a]aresample=44100,aformat=channel_layouts=stereo[a0];[1:a]aresample=44100,aformat=channel_layouts=stereo[a1];[v0][a0][v1][a1]concat=n=2:v=1:a=1[v][a]`
      : `[0:v]fps=30,format=yuv420p,setsar=1[v0];[1:v]fps=30,format=yuv420p,setsar=1[v1];[v0][v1]concat=n=2:v=1[v]`
    const map = meta.hasAudio ? ['-map', '[v]', '-map', '[a]', '-c:a', 'aac', '-b:a', '128k'] : ['-map', '[v]']
    // card first, then body
    await run('ffmpeg', ['-i', card, '-i', bodyFile, '-filter_complex', va, ...map, '-c:v', 'libx264', '-preset', 'fast', '-crf', '21', '-movflags', '+faststart', '-y', merged], { timeoutMs: 4 * 60 * 1000 })
    return merged
  } catch { return bodyFile }
}
...
export async function processClipJob(job) {
  ...
  try {
    ...
    // On-video copy (Claude), then rotate clips through the user's chosen
    // edit formats. Card formats also get platform-native tweet/reddit copy.
    const edits = (Array.isArray(job.edit_formats) && job.edit_formats.length ? job.edit_formats : ['captions'])
      .filter(e => EDIT_FORMATS.some(f => f.key === e))
    await setDetail(job.id, 'Writing the copy…')
    const copy = await writeClipCopy(picks, { sourceName: job.source_name, segments, wantCards: edits.some(e => CARD_EDITS.includes(e)) })
    picks = picks.map((p, i) => ({ ...p, ...(copy[i] || {}), title: copy[i]?.title || p.title || `Clip ${i + 1}` }))
    ...
    for (let i = 0; i < picks.length; i++) {
      ...
      // ASS layer: captions need word timestamps; hook titles + legacy brand
      // pills/endcards render even without a transcript. Card formats get
      // their PNG either way — only the captions depend on words.
      let assFile = null
      if (ASS_EDITS.includes(edit) && (tr?.words?.length || ['hook', 'brand'].includes(edit))) {
        const ass = buildKaraokeAss({
          words: tr?.words || [], clipStart: p.start, clipEnd: p.end,
          alignment: edit === 'sludge' ? 5 : 2,
          marginV: edit === 'sludge' ? 0 : 460,
          title: edit === 'hook' ? p.title : null,
          variant: edit === 'emphasis' ? 'emphasis' : edit === 'cinematic' ? 'cinematic' : 'karaoke',
          brandHandle: edit === 'brand' ? (job.watermark || '') : null,
        })
      ...
      // Social-card overlays (tweet / thread / reddit) — rendered PNGs, real
      // anatomy. A failed render degrades to plain captions, never a dead job.
      const cards = [] // { file, y, enable? }
      if (CARD_EDITS.includes(edit)) {
        const handle = String(job.watermark || 'cadence').replace(/^@/, '').slice(0, 24)
        const brandName = handle.charAt(0).toUpperCase() + handle.slice(1)
        const logoUrl = job.outro_logo_url || null
        try {
          if (edit === 'tweet') {
            const f = path.join(dir, `card${i}-0.png`)
            await writeFile(f, await renderTweetCard({ name: brandName, handle, text: p.tweet || p.title, logoUrl }))
            cards.push({ file: f, y: 96 })
          } else if (edit === 'thread') {
            const parts = (Array.isArray(p.thread) && p.thread.length ? p.thread : [p.tweet || p.title]).slice(0, 3)
            const seg = durSec / parts.length
            for (let k = 0; k < parts.length; k++) {
              const f = path.join(dir, `card${i}-${k}.png`)
              await writeFile(f, await renderTweetCard({ name: brandName, handle, text: parts[k], logoUrl }))
              cards.push({ file: f, y: 96, enable: `between(t,${(k * seg).toFixed(2)},${(k === parts.length - 1 ? durSec : (k + 1) * seg).toFixed(2)})` })
            }
          } else if (edit === 'reddit') {
            const f = path.join(dir, `card${i}-0.png`)
            await writeFile(f, await renderRedditCard({ subreddit: p.subreddit, author: handle, title: p.reddit_title || p.title }))
            // Story-time pattern: the card opens the clip, then drops away and
            // the captions carry it.
            cards.push({ file: f, y: 150, enable: `lt(t,${Math.min(4.5, durSec * 0.45).toFixed(2)})` })
          }
        } catch { cards.length = 0 }
      }
      ...
      const buildArgs = (withSubs) => {
        ...
        if (edit === 'sludge' && sludgeFiller) {
          ...
          chain = `[0:v]scale=1080:960:force_original_aspect_ratio=increase,crop=1080:960[top];` +
            ...
            editOverlays('clean', { watermark: job.watermark, durSec }).post
        } else {
          cards.forEach(c => a.push('-i', c.file))
          const base = formatFilter(job.format, false) || '[0:v]null'
          // Boxes draw BEFORE the ASS layer; card PNGs overlay next (still
          // under the captions); legacy drawtext + watermark draw after.
          const { pre, post } = editOverlays(edit, { titleFile, watermark: job.watermark, durSec })
          chain = base + pre
          cards.forEach((c, k) => {
            chain += `[cv${i}_${k}];[cv${i}_${k}][${k + 1}:v]overlay=(main_w-overlay_w)/2:${c.y}${c.enable ? `:enable='${c.enable}'` : ''}`
          })
          chain += (withSubs ? subs : '') + post
        }
      ...
      // Cold open: prepend the bold hook card in front of the captioned body.
      let finalFile = outFile
      if (edit === 'cold_open') {
        await setDetail(job.id, `Adding the cold-open hook to clip ${i + 1}…`)
        finalFile = await prependColdOpen(finalFile, dir, i, { hook: p.cold_open || p.title })
      }

      // Universal brand outro: logo + chime endcard appended to EVERY clip
      // when the job asks for it (independent of edit style).
      if (job.outro) {
        await setDetail(job.id, `Adding the outro to clip ${i + 1}…`)
        finalFile = await appendOutro(outFile, dir, i, { logoUrl: job.outro_logo_url, handle: job.watermark })
      }

      await setDetail(job.id, `Uploading clip ${i + 1}…`)
      const storagePath = `${job.user_id}/${job.id}/clip-${i}.mp4`
      const { error: upErr } = await admin.storage.from('clips').upload(storagePath, await readFile(finalFile), { contentType: 'video/mp4', upsert: true, cacheControl: '31536000' })
    ...
    // Campaign auto-post: a clip job created by a brand campaign publishes its
    // best clip straight to the target account(s) and records provenance.
    if (job.auto_post && Array.isArray(job.account_ids) && job.account_ids.length && clips.length) {
      try { await autoPostClip(job, clips[0]) }
      catch (e) { await admin.from('clip_jobs').update({ status_detail: `${clips.length} clips · auto-post failed: ${String(e.message).slice(0, 80)}` }).eq('id', job.id) }
    }
...
// Publish a campaign clip to its target IG/TikTok accounts + record a posts row
// so it shows up in the queue/history with campaign provenance.
async function autoPostClip(job, clip) {
  if (!zernioEnabled()) throw new Error('publishing not connected')
  const { data: accts } = await admin.from('social_accounts').select('*')
    .eq('user_id', job.user_id).in('id', job.account_ids).in('platform', ['instagram', 'tiktok'])
  if (!accts?.length) throw new Error('no connected accounts')
  const caption = clip.caption || clip.title || ''
  const r = await createPost({ userId: job.user_id, accounts: accts, content: caption, videoUrl: clip.url, title: clip.title })
  const nowIso = new Date().toISOString()
  await admin.from('posts').insert(accts.map(a => ({
    user_id: job.user_id, platform: a.platform, content: caption, status: 'posted',
    source: 'campaign', brand_campaign_id: job.brand_campaign_id || null,
    external_id: r.id, posted_at: nowIso, scheduled_for: nowIso,
  })))
}
