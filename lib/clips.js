...
//   1. download + ffprobe validate (caps: 45 min / ~500MB)
...
// ── Edit formats — the "traditional edit" overlays ─────────────────────────────
// Grounded in what carries short-form in 2026: text overlays (most viewing is
// muted), a hook in the first seconds, light-handed styling. Users pick a
// SUBSET per job/campaign; clips rotate through the chosen formats.
export const EDIT_FORMATS = [
  { key: 'meme_bar', label: 'Meme bar', desc: 'White headline bar on top with a punchy title — the classic shareable' },
  { key: 'hook', label: 'Hook flash', desc: 'Big bold title for the first 3.5s, then clean video' },
  { key: 'banner', label: 'Lower banner', desc: 'Podcast-style bottom banner with the title + your handle' },
  { key: 'progress', label: 'Progress bar', desc: 'Clean video with an animated progress bar (retention cue)' },
  { key: 'clean', label: 'Clean', desc: 'Just your watermark' },
]

// First usable bold-ish system font; override with CLIP_FONT for deploys.
...
// Whisper transcription (only when OPENAI_API_KEY is configured).
async function transcribe(file, dir) {
  ...
  if (!res.ok) return null // transcription is an upgrade, never a blocker
  const j = await res.json()
  return (j.segments || []).map(s => ({ start: s.start, end: s.end, text: (s.text || '').trim() }))
}

// Claude picks the hook-worthy moments from the transcript.
async function pickHighlights(segments, n, [minL, maxL]) {
  ...
  const res = await anthropic.messages.create({
    ...
    system: `You pick the most scroll-stopping moments from a video transcript for short-form clips (Reels/TikTok). Each clip must be ${minL}-${maxL} seconds, self-contained (a hook, a payoff), and start/end at natural sentence boundaries from the given timestamps. Also write a punchy title (<=8 words) and a 1-2 sentence caption for each.
...
async function setDetail(id, status_detail) { await admin.from('clip_jobs').update({ status_detail }).eq('id', id) }
...
async function ingest(job, src) {
  ...
  let pageDuration = 0
  try {
    const { out } = await run('yt-dlp', ['--no-playlist', '--print', 'duration', job.source_url], { timeoutMs: 90000 })
    pageDuration = Number(out.trim().split('\n').pop()) || 0
  } catch (e) {
    const msg = String(e.message || '')
    if (/private|sign in|login|age/i.test(msg)) throw new Error('That video is private or login-gated — Cadence can only clip public videos.')
    throw new Error(`Could not read that video page: ${msg.slice(-220)}`)
  }
...
export async function processClipJob(job) {
  ...
  try {
    ...
    if (meta.duration > MAX_DURATION) throw new Error('Video is over 45 minutes — clip something shorter.')
    ...
    // Highlight selection: transcript+Claude when possible, else audio energy.
    await setDetail(job.id, 'Finding the best moments…')
    const segments = await transcribe(src, dir)
    let picks = []
    if (segments?.length) {
    ...
    for (let i = 0; i < picks.length; i++) {
      ...
      let subFile = null
      if (job.captions && segments?.length) subFile = await srtFor(segments, p.start, p.end, path.join(dir, 'subs.srt'))
      // Title text via a file: escaping- and linebreak-safe for drawtext.
      const titleFile = path.join(dir, `title${i}.txt`)
      await writeFile(titleFile, wrapTitle(p.title))
      const overlay = editFilter(edit, { titleFile, watermark: job.watermark, durSec: p.end - p.start })

      const buildArgs = withSubs => {
        const base = formatFilter(job.format, withSubs)
        const chain = base ? base + overlay : (overlay ? `[0:v]null${overlay}` : null)
        const a = ['-ss', String(p.start), '-to', String(p.end), '-i', src]
        if (chain) a.push('-filter_complex', chain)
        a.push('-c:v', 'libx264', '-preset', 'fast', '-crf', '21', '-c:a', 'aac', '-b:a', '128k', '-movflags', '+faststart', '-y', outFile)
        return a
      }
      // Subtitle/drawtext filters can fail on odd fonts/paths — retry plainer.
      await run('ffmpeg', buildArgs(!!subFile), { timeoutMs: 8 * 60 * 1000 })
      ...
      const { error: upErr } = await admin.storage.from('clips').upload(storagePath, await readFile(outFile), { contentType: 'video/mp4', upsert: true })
...
export async function processQueuedClipJobs(maxJobs = 2) {
  // Recover jobs orphaned mid-processing (e.g. server restart): if one has been
  // "processing" for over an hour with no fresh status, put it back in line.
  const stale = new Date(Date.now() - 60 * 60 * 1000).toISOString()
  await admin.from('clip_jobs').update({ status: 'queued', status_detail: 'Retrying…' })
    .eq('status', 'processing').lt('created_at', stale)

  const out = []
  for (let i = 0; i < maxJobs; i++) {
    const { data: job } = await admin.from('clip_jobs').select('*').eq('status', 'queued').order('created_at').limit(1).single()
    if (!job) break
    out.push(await processClipJob(job))
  }
  return out
}
