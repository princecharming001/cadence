...
export async function processClipJob(job) {
  ...
  try {
    ...
    if (!picks.length) {
      ...
      await setDetail(job.id, `Scoring ${wins.length} candidate moments…`)
      const scored = []
      for (const w of wins.slice(0, 24)) scored.push({ w, score: await energyScore(src, w) })
    ...
    for (let i = 0; i < picks.length; i++) {
      ...
      const filter = formatFilter(job.format, !!subFile)
      const args = ['-ss', String(p.start), '-to', String(p.end), '-i', src]
      if (filter) args.push('-filter_complex', filter)
      args.push('-c:v', 'libx264', '-preset', 'fast', '-crf', '21', '-c:a', 'aac', '-b:a', '128k', '-movflags', '+faststart', '-y', outFile)
      await run('ffmpeg', args, { timeoutMs: 8 * 60 * 1000 }).catch(async () => {
        // Subtitle filter can fail on odd fonts/paths — retry without captions.
        const plain = formatFilter(job.format, false)
        const a2 = ['-ss', String(p.start), '-to', String(p.end), '-i', src]
        if (plain) a2.push('-filter_complex', plain)
        a2.push('-c:v', 'libx264', '-preset', 'fast', '-crf', '21', '-c:a', 'aac', '-b:a', '128k', '-movflags', '+faststart', '-y', outFile)
        await run('ffmpeg', a2, { timeoutMs: 8 * 60 * 1000 })
      })
      ...
      clips.push({
        ...
        title: p.title || `Clip ${i + 1}`, caption: p.caption || '',
      })
    }
