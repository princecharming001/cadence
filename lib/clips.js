...
function buildKaraokeAss({ words, clipStart, clipEnd, alignment = 2, marginV = 480, fontSize = 92 }) {
...
async function ingest(job, src) {
  ...
  // you're not a bot") — that is NOT a private video. Recovery ladder: plain
  // request, then the local browser's own YouTube session cookies.
  ...
  const runYt = async (args, timeoutMs) => {
    const ladders = [[], ['--cookies-from-browser', 'chrome'], ['--cookies-from-browser', 'safari']]
    let lastErr
    ...
    if (isBotCheck(String(lastErr?.message || ''))) {
      throw new Error('YouTube is rate-limiting downloads from this machine right now (its bot check). Try again in a few minutes — being signed into YouTube in Chrome or Safari helps Cadence get through.')
    }
    throw lastErr
  }
...
export async function processClipJob(job) {
  ...
  try {
    ...
    for (let i = 0; i < picks.length; i++) {
      ...
      let assFile = null
      if (tr?.words?.length && ['captions', 'sludge', 'hook'].includes(edit)) {
      ...
      const buildArgs = (withSubs) => {
        ...
        if (edit === 'sludge' && sludgeFiller) {
          ...
          // 'hook' renders the title flash on top of captions; 'captions' is
          // captions + watermark; legacy keys keep their old overlays.
          const overlay = editFilter(['captions', 'sludge'].includes(edit) ? 'clean' : edit, { titleFile, watermark: job.watermark, durSec })
