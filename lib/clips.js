...
export const EDIT_FORMATS = [
  ...
  { key: 'emphasis', label: 'Emphasis pop', desc: 'Huge center captions, every line pops in a new color — the founder-clip standard' },
  { key: 'tweet', label: 'Tweet frame', desc: 'Clip framed under a viral-tweet card with your handle — built for shares' },
  { key: 'brand', label: 'Brand + follow', desc: 'Your handle pinned on screen, ends on a big FOLLOW card' },
  { key: 'cinematic', label: 'Cinematic', desc: 'Letterbox bars + minimal elegant captions — premium story feel' },
...
// Formats whose text renders through the ASS/libass layer (captions and/or
// static card text). tweet/brand still get their card even with no transcript.
const ASS_EDITS = ['captions', 'sludge', 'hook', 'emphasis', 'tweet', 'cinematic', 'brand']
...
function editOverlays(edit, { titleFile, watermark, durSec }) {
  ...
  switch (edit) {
    case 'tweet':
      // White "viral tweet screenshot" card up top; handle + title text render
      // in the ASS layer above it. No corner watermark — the handle IS the card.
      return { pre: `,drawbox=x=54:y=110:w=iw-108:h=330:color=white:t=fill,drawbox=x=54:y=110:w=iw-108:h=8:color=0x1D9BF0:t=fill`, post: '' }
    case 'cinematic':
...
function buildKaraokeAss({ words, clipStart, clipEnd, alignment = 2, marginV = 480, fontSize = 92, title = null, variant = 'karaoke', tweetCard = null, brandHandle = null }) {
  const dur = clipEnd - clipStart
  const inRange = (words || []).filter(w => w.start >= clipStart - 0.2 && w.end <= clipEnd + 0.2)
  if (!inRange.length && !title && !tweetCard && brandHandle == null) return null
  ...
  // Tweet frame: handle + hook text laid over the white card the filter drew.
  if (tweetCard) {
    const handle = clean(String(tweetCard.handle || '').replace(/^@/, ''))
    const t = wrapTitle(tweetCard.title || '', 30).split('\n').map(clean).join('\\N')
    events.push(`Dialogue: 1,${assTime(0)},${assTime(dur)},TweetHandle,,0,0,0,,${handle ? `@${handle}` : 'on the record'}`)
    if (t) events.push(`Dialogue: 1,${assTime(0)},${assTime(dur)},TweetBody,,0,0,0,,${t}`)
  }
...
Style: TweetHandle,Anton,34,&H00F09B1D,&H00F09B1D,&H00FFFFFF,&H00FFFFFF,-1,0,0,0,100,100,1,0,1,0,0,7,110,110,150,1
Style: TweetBody,Anton,46,&H00181818,&H00181818,&H00FFFFFF,&H00FFFFFF,-1,0,0,0,100,100,1,0,1,0,0,7,110,110,205,1
...
export async function processClipJob(job) {
  ...
  try {
    ...
    for (let i = 0; i < picks.length; i++) {
      ...
      // ASS layer: karaoke/emphasis/cinematic captions need word timestamps;
      // tweet cards + brand pills/endcards render even without a transcript.
      let assFile = null
      if (ASS_EDITS.includes(edit) && (tr?.words?.length || ['hook', 'tweet', 'brand'].includes(edit))) {
        const ass = buildKaraokeAss({
          words: tr?.words || [], clipStart: p.start, clipEnd: p.end,
          alignment: edit === 'sludge' ? 5 : 2,
          marginV: edit === 'sludge' ? 0 : edit === 'tweet' ? 560 : 460,
          title: edit === 'hook' ? p.title : null,
          variant: edit === 'emphasis' ? 'emphasis' : edit === 'cinematic' ? 'cinematic' : 'karaoke',
          tweetCard: edit === 'tweet' ? { handle: job.watermark || '', title: p.title } : null,
      ...
      const buildArgs = (withSubs) => {
        ...
        if (edit === 'sludge' && sludgeFiller) {
          ...
          // Boxes/cards draw BEFORE the ASS layer (text sits on top of them);
          // legacy drawtext overlays + the corner watermark draw after.
          const { pre, post } = editOverlays(edit, { titleFile, watermark: job.watermark, durSec })
          chain = base + pre + (withSubs ? subs : '') + post
        }
