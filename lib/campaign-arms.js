...
// For the dashboard: the leading arm per dimension per platform, with the
// uncertainty + lift signals the operator needs to NOT over-trust it — posterior
// std (wide = weak signal), how far the arm has separated from its prior (≈0 means
// the bandit hasn't actually learned anything yet), and a `weak` flag.
export async function topArms(campaignId) {
  ...
  const byKey = {}
  for (const r of data || []) {
    const a = Math.max(0.01, r.alpha), b = Math.max(0.01, r.beta), c = a + b
    const mean = a / c
    const std = Math.sqrt((a * b) / (c * c * (c + 1)))
    const [pa, pb] = priorFor(r.dimension, r.value)
    const lift = mean - pa / (pa + pb) // separation from the arm's starting belief
    const k = `${r.platform}:${r.dimension}`
    if (!byKey[k] || mean > byKey[k].mean) {
      byKey[k] = {
        platform: r.platform, dimension: r.dimension, value: r.value,
        mean: +mean.toFixed(3), std: +std.toFixed(3), lift: +lift.toFixed(3), obs: r.obs,
        weak: (r.obs || 0) < 5 || std > 0.15 || Math.abs(lift) < 0.03,
      }
    }
  }
