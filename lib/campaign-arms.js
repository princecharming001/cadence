...
export async function recordArmOutcome(campaignId, platform, dimension, value, success, rewardRate) {
  ...
  // Recency decay toward prior (social is non-stationary) — gentler 0.85^weeks
  // (~4.3-week half-life) so hard-won learning isn't erased before it compounds in
  // a low-velocity campaign, while a months-stale winner still fades.
  if (existing?.updated_at) {
    const weeks = (Date.now() - new Date(existing.updated_at).getTime()) / (7 * 864e5)
    if (weeks > 0.5) { const f = Math.pow(0.85, weeks); alpha = pa + (alpha - pa) * f; beta = pb + (beta - pb) * f }
  }
