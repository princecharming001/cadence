...
async function processRule(r) {
  ...
  for (let j = 0; j < batch.length; j++) {
    ...
    // auto_post=true -> queued for NOW (immediate). Replies ride the post's
    // algorithmic wave — the earlier the reply lands, the more reach it gets,
    // so we do NOT stagger; the next cron tick (≤1 min) publishes them all.
    const row = {
