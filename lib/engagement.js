...
// Write a live status line on the rule so the UI can show what it's doing now.
async function setStatus(id, detail, running) {
  ...
  await admin.from('engagement_rules').update(patch).eq('id', id)
}

// Run one engagement rule end to end, reporting progress as it goes.
async function processRule(r) {
  const nowMs = Date.now()
  const nowIso = new Date(nowMs).toISOString()
  const intervalMs = (Number(r.interval_hours) || 24) * 3600 * 1000
  const bumpNext = () => admin.from('engagement_rules')
    .update({ last_run_at: nowIso, next_run_at: new Date(nowMs + intervalMs).toISOString(), running: false }).eq('id', r.id)
...
export async function runDueEngagement() {
  ...
  if (!rules?.length) return { rules: 0, results: [] }
  const results = []
  for (const r of rules) {
    try { results.push(await processRule(r)) }
    catch (e) { await setStatus(r.id, `Error: ${e.message}`.slice(0, 200), false); results.push({ id: r.id, error: e.message }) }
  }
  return { rules: rules.length, results }
}

// Run a single rule now, ignoring its schedule (the "Run now" button). Scoped
// to the owner so a user can only trigger their own rules.
export async function runEngagementById(id, userId) {
  const { data: r } = await admin.from('engagement_rules').select('*').eq('id', id).eq('user_id', userId).single()
  if (!r) return { error: 'not found' }
  try { return await processRule(r) }
  catch (e) { await setStatus(r.id, `Error: ${e.message}`.slice(0, 200), false); return { id: r.id, error: e.message } }
}
