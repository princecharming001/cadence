...
import { xReadEnabled, searchRecent, getValidAccessToken, postTweet, canReplyTo } from './x-oauth'
...
export async function discoverTargets(rule, conn) {
  ...
  // De-dup + drop tweets whose author limited replies (a reply there would 403),
  // then rank by viral velocity weighted by relevance.
  const seen = new Set()
  return candidates
    .filter(t => t.tweet_id && canReplyTo(t.reply_settings) && !seen.has(t.tweet_id) && seen.add(t.tweet_id))
...
Respond with ONLY the reply text, nothing else.`,
  ...
  // Drop LLM refusals/meta — they must NEVER go out as a public reply
  // ("This isn't a topic I can engage with…", "As an AI…", etc.).
  if (/\b(i (can'?t|cannot|won'?t|am not able)|'?m unable|don'?t feel comfortable|isn'?t (a topic|something) i can|as an ai|i'?m an ai language model|i'?m just an ai)\b/i.test(text)) return ''
...
async function processRule(r) {
  ...
  if (!conn) {
    // The rule's stored account is gone (disconnected/switched) → fall back to the
    // ACTIVE (primary) account, not an arbitrary oldest one, so niche replies fire
    // from the account the user is actually managing.
    const { data } = await admin.from('x_connections').select('*').eq('user_id', r.user_id)
  ...
  const token = auto ? await getValidAccessToken(conn) : null
  ...
  for (let j = 0; j < batch.length; j++) {
    ...
    const row = {
      ...
      x_connection_id: conn.id, engagement_rule_id: r.id, scheduled_for: nowIso,
      reply_to_tweet_id: t.tweet_id, target_tweet_text: t.text || null, target_tweet_url: t.url || null,
    }
    if (auto) {
      // Niche replies post the MOMENT we read a relevant post — replying while
      // the post is hot is the whole point (ride its algorithmic wave). Posted
      // directly, not queued. The per-reply write time spaces the batch
      // naturally (no robotic same-second burst).
      try {
        const ext = await postTweet(token, text, null, t.tweet_id)
        await admin.from('posts').insert({ ...row, status: 'posted', external_id: ext || null, posted_at: new Date().toISOString() })
        made++
      } catch (e) {
        // The Supabase query builder is a thenable with NO .catch() — using
        // `.insert(...).catch()` threw synchronously and crashed the whole run.
        try { await admin.from('posts').insert({ ...row, status: 'failed', error: String(e.message || '').slice(0, 200) }) } catch {}
      }
    } else {
      const { error } = await admin.from('posts').insert({ ...row, status: 'draft' })
      if (!error) made++
    }
    // unique-index conflicts (someone replied meanwhile) are silently fine
  }

  await setStatus(r.id, auto ? `Replied to ${made}` : `Drafted ${made} repl${made === 1 ? 'y' : 'ies'} for approval`, false)
