...
// Read cached rows that MATCH the query (tag-overlap OR phrase substring),
// least-used first. Never returns unrelated rows: a query with no usable tags
// still filters on the phrase, so a topic with no cache hit returns [] (→ fetch).
async function fromCache(query, type, n) {
  ...
  if (ts.length) q = q.or(`tags.ov.{${ts.join(',')}},query.ilike.%${phrase}%`)
  else if (phrase) q = q.ilike('query', `%${phrase}%`)
  else return []
...
export async function searchStock(query, { type = 'video', n = 4, orientation = 'portrait' } = {}) {
  ...
  hits = hits.slice(0, n)
  // Per-row increment (a single shared value would flatten the LRU rotation).
  const now = new Date().toISOString()
  await Promise.all(hits.map(h => admin.from('stock_assets').update({ used_count: (h.used_count || 0) + 1, last_used_at: now }).eq('id', h.id).then(() => {}, () => {})))
