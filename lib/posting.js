...
async function postOneZernio(post) {
  try {
    ...
    if (post.social_account_id) {
      ...
      // No pinned account → publish as the ACTIVE account for this platform (not an
      // arbitrary first row), so an un-pinned post lands on the managed account.
      const { data } = await admin.from('social_accounts').select('*')
        .eq('user_id', post.user_id).eq('platform', post.platform)
