...
async function postOneZernio(post) {
  try {
    ...
    // Carousels carry every slide in image_urls; single-image posts keep image_url.
    const media = Array.isArray(post.image_urls) && post.image_urls.length
      ? post.image_urls
      : (post.image_url ? [post.image_url] : [])
    if ((post.platform === 'instagram' || post.platform === 'tiktok') && !media.length) {
      throw new Error(`${post.platform} posts need an image.`)
    }
