...
// X branch: replies to the user's posts ARE tweets replying to them. Read them
// via the X API (to:username), reply in voice with the user's own token.
async function runXEngagement(userId, setting, persona) {
  if (!xReadEnabled()) { await setStatus(setting.id, 'X reading is off (X_READ_ENABLED)', false); return { error: 'X reading is off' } }
  ...
  let replies = []
  try { replies = await searchRecent(token, `to:${conn.username} -from:${conn.username} -is:retweet`, 25) }
  ...
  for (const t of replies) {
    if (!t.tweet_id || seen.has(t.tweet_id)) continue
    seen.add(t.tweet_id)
    await setStatus(setting.id, `Replying to @${t.author || 'someone'}…`, true)
    let reply_text = ''
    try { reply_text = await writeReply({ platform: 'x', comment: { text: t.text, authorUsername: t.author }, postCaption: '', persona, instructions: setting.instructions }) }
    catch { continue }
    ...
    const row = {
      ...
      reply_text: reply_text.slice(0, 280), status: 'draft',
    }
    if (setting.auto_post) {
      try { await postTweet(token, row.reply_text, null, t.tweet_id); row.status = 'posted'; posted++ }
...
export async function runSocialEngagement(userId, platform) {
  ...
  if (platform === 'x') {
    ...
    try { return await runXEngagement(userId, setting, persona) }
    catch (e) { await setStatus(setting.id, `Error: ${e.message}`, false); return { error: e.message } }
  }
