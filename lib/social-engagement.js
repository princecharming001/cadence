...
// Comment triage: humans don't reply to every "🔥" — and replying to all of them
// is a bot tell that wastes the per-run budget. Score so the budget goes to the
// comments worth a real reply (questions + substantive), and skip pure junk.
const LINK_RE = /https?:\/\/|www\.[a-z0-9-]/i
const SPAM_RE = /\b(follow me|follow back|check my|dm me|sub to|link in (bio|profile)|promo code|free followers|cash ?app|onlyfans)\b/i
const isQuestion = t => /\?/.test(t) || /^(how|what|why|when|where|who|which|can|could|do|does|is|are|should|any)\b/i.test(String(t || '').trim())
function commentScore(text) {
  const t = String(text || '').trim()
  if (t.length < 3 || LINK_RE.test(t) || SPAM_RE.test(t)) return -1 // junk/spam → skip
  const words = t.replace(/[^\p{L}\p{N}\s]/gu, ' ').split(/\s+/).filter(Boolean)
  if (words.length < 2) return -1 // emoji-only / one-word → not worth a reply
  let s = Math.min(t.length, 220) / 220 // longer = more substantive
  if (isQuestion(t)) s += 1 // a question deserves an answer — top priority
  return s
}
// Sort a comment list best-first and drop the junk, so the cap is spent well.
const triage = (list, textOf) => list
  .map(c => ({ c, s: commentScore(textOf(c)) }))
  .filter(x => x.s >= 0)
  .sort((a, b) => b.s - a.s)
  .map(x => x.c)

// Per-commenter relationship memory: recall prior exchanges with this author so a
// repeat fan gets continuity, not a cold one-shot reply every time.
async function authorHistory(userId, platform, author) {
  if (!author) return ''
  const { data } = await admin.from('social_replies')
    .select('comment_text, reply_text, created_at')
    .eq('user_id', userId).eq('platform', platform).eq('comment_author', author).not('reply_text', 'is', null)
    .order('created_at', { ascending: false }).limit(3)
  const rows = (data || []).filter(r => r.reply_text)
  if (!rows.length) return ''
  return `\n\nYOU'VE TALKED WITH @${author} BEFORE (acknowledge the ongoing relationship naturally if it fits; never repeat a reply you already gave them):\n${rows.reverse().map(r => `- they said "${String(r.comment_text || '').slice(0, 90)}" → you replied "${String(r.reply_text || '').slice(0, 90)}"`).join('\n')}`
}

async function writeReply({ platform, comment, postCaption, persona, fb, instructions, userId }) {
  const history = userId ? await authorHistory(userId, platform, commentAuthor(comment)).catch(() => '') : ''
...
${REPLY_RUBRIC}${feedbackBlock(fb)}${history}
...
async function runXEngagement(setting, persona, fb) {
  ...
  for (const t of triage(replies, r => r.text)) { // best/most-answerable first; junk skipped
    ...
    try {
      const reply_text = await writeReply({ platform: 'x', comment: { text: t.text, authorUsername: t.author }, postCaption: '', persona, fb, instructions: setting.instructions, userId })
      ...
      if (setting.auto_post) {
        // These are replies to comments on the user's OWN posts. An instant
        // reply reads as a bot — so we wait a human 30-90s: queue the reply and
        // let the cron precision-fire it. (Niche replies to OTHERS' posts stay
        // immediate to ride that post's algorithmic wave — see lib/engagement.js.)
        const delayMs = (30 + Math.random() * 60) * 1000
        const { error: qErr } = await admin.from('posts').insert({
          user_id: userId, content: reply_text, source: 'reply', platform: 'x',
          status: 'queued', scheduled_for: new Date(Date.now() + delayMs).toISOString(),
          x_connection_id: conn.id, reply_to_tweet_id: t.tweet_id,
          target_tweet_text: t.text || null, target_tweet_url: t.url || null,
        })
        if (qErr) { await admin.from('social_replies').update({ status: 'failed', error: String(qErr.message || '').slice(0, 180) }).eq('id', gateId); continue }
        await admin.from('social_replies').update({ reply_text, status: 'queued' }).eq('id', gateId)
        posted++
      } else {
  ...
  await setStatus(setting.id, posted || drafted ? `Done — ${posted} queued, ${drafted} drafted` : 'No new replies', false)
...
async function runZernioEngagement(setting, persona, fb) {
  ...
  for (const acct of accts) {
    ...
    for (const post of commentedPosts) {
      ...
      for (const c of triage(comments, commentText)) { // prioritize questions/substantive, skip junk
        if (posted + drafted >= cap) break
        ...
        try {
          const reply_text = await writeReply({ platform, comment: c, postCaption: pick(post, ['content', 'caption', 'text']), persona, fb, instructions: setting.instructions, userId })
          if (!reply_text) { await admin.from('social_replies').update({ status: 'failed', error: 'empty draft' }).eq('id', gateId); continue }
          if (setting.auto_post) {
...
async function runUnipileLinkedIn(setting, persona, fb, accountId) {
  ...
  for (const post of posts) {
    ...
    for (const c of triage(comments, c => c.text || c.message)) { // questions/substantive first, junk skipped
      if (posted + drafted >= cap) break
      ...
      try {
        const reply_text = await writeReply({ platform: 'linkedin', comment: { text: c.text || c.message, authorUsername: author }, postCaption: post.text || post.commentary || '', persona, fb, instructions: setting.instructions, userId })
        if (!reply_text) { await admin.from('social_replies').update({ status: 'failed', error: 'empty draft' }).eq('id', gateId); continue }
        if (setting.auto_post) {
