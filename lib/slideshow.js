...
import { voiceBlock } from './prompts'
...
export async function outlineSlideshow({ topic, format = 'listicle', slides = 6, persona, handle, memory = '', hook = '' }) {
  ...
  // Full voice (summary/signature-moves/sample-hooks), not a flattened 3-field
  // line — the carousel should sound as much like the person as a post does.
  const voice = persona
    ? voiceBlock(persona, { register: 'longform' })
