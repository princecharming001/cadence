...
export async function outlineSlideshow({ topic, format = 'listicle', slides = 6, persona, handle, memory = '' }) {
...
export async function generateSlideshow({ topic, format, style = 'bold', slides = 6, persona, handle, userId, albumIds, memory = '' }) {
  const styleDef = SLIDE_STYLES[style] || SLIDE_STYLES.bold
  const outline = await outlineSlideshow({ topic, format, slides, persona, handle, memory })
