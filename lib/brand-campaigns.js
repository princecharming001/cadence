...
async function writePost({ topic, platform, persona, fb, recent }) {
...
${PROMO_RUBRIC}${feedbackBlock(fb)}${antiRepetition(recent)}
...
async function processBrandCampaign(c) {
  ...
  try {
    ...
    const { persona, fb, recent } = await getVoice(c.user_id)

    // Schedule a few minutes out so the user can see (and stop) what's coming.
    ...
    for (const t of targets) {
      try {
        if (t.kind === 'x') {
          ...
          const text = await writePost({ topic: c.topic, platform: 'x', persona, fb, recent })
          let imageUrl = null
          if (c.include_image) {
            try {
              const img = await generateImage(text, { fromContent: true })
              // Don't attach a stock placeholder; persist real images so they
              // survive until the post publishes.
              if (!img.placeholder) imageUrl = await persistImage(img.url, c.user_id)
            } catch {}
          }
          ...
          const text = await writePost({ topic: c.topic, platform: 'linkedin', persona, fb, recent })
