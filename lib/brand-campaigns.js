...
import { PROMO_RUBRIC, X_RUBRIC, LINKEDIN_RUBRIC, enforceLen } from './prompts'
...
// One on-voice promo post for a platform, through the full shared brain. `mem` is
// getBrandMemory() (voice + brief + durable insights + feedback + winners +
// cross-account coherence); `trends` is this platform's live trending block.
async function writePost({ topic, platform, mem, trends }) {
...
${PROMO_RUBRIC}${mem.memoryBlock({ withTrends: false })}${trends || ''}${mem.antiRepetition()}
...
async function processBrandCampaign(c) {
  ...
  try {
    ...
    // winners + cross-account coherence). Trends are fetched per-platform below.
    const mem = await getBrandMemory(c.user_id, { includeTrends: false })
    ...
    for (const t of targets) {
      try {
        if (t.kind === 'x') {
          ...
          const text = await writePost({ topic: c.topic, platform: 'x', mem, trends: await trendingBlock(c.user_id, 'x').catch(() => '') })
          let imageUrl = null
          if (c.include_image) {
            try {
          ...
          const text = await writePost({ topic: c.topic, platform: 'linkedin', mem, trends: await trendingBlock(c.user_id, 'linkedin').catch(() => '') })
          ...
          const deck = await generateSlideshow({ topic: c.topic, format: c.carousel_format, style: c.carousel_style, slides: 6, persona: mem.persona, userId: c.user_id, albumIds: c.album_ids, memory: mem.memoryBlock({ withContext: false, withTrends: false }) })
          const { data: acct } = await admin.from('social_accounts').select('*').eq('id', t.id).eq('user_id', c.user_id).single()
          const row = {
