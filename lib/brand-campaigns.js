...
async function writePost({ topic, platform, persona, fb, recent, brandCtx }) {
...
${PROMO_RUBRIC}${feedbackBlock(fb)}${brandCtx || ''}${antiRepetition(recent)}
...
async function processBrandCampaign(c) {
  ...
  try {
    ...
    const [{ persona, fb, recent }, brandCtx] = await Promise.all([getVoice(c.user_id), brandContextBlock(c.user_id)])
    ...
    for (const t of targets) {
      try {
        if (t.kind === 'x') {
          ...
          const text = await writePost({ topic: c.topic, platform: 'x', persona, fb, recent, brandCtx })
          let imageUrl = null
          if (c.include_image) {
            try {
          ...
          const text = await writePost({ topic: c.topic, platform: 'linkedin', persona, fb, recent, brandCtx })
