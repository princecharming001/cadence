...
async function rewardArms(campaignId, signal) {
  ...
  for (const p of trusted) {
    // STRICT '>' — the median was computed from these same posts, so '>=' would let
    // the median post pass against a threshold it itself set (data leakage), biasing
    // every arm toward a win. Only posts strictly above the campaign median win.
