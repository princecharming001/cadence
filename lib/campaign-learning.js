...
async function gatherSignal(campaignId, userId) {
  ...
  const timing = timingConfound(top)
  const summary = await campaignSentimentSummary(campaignId).catch(() => ({ total: 0 }))
  return { top, bottom, summary, total: rows.length, useRate, scored, medianRate, medianRateByPlatform, floors, timing }
}

// Time-of-day is a confounder reach-normalization doesn't fully remove: if the
// best posts all went out in the same window, some of the "win" is posting-TIME
// luck, not the angle. We don't try to regress it out (too noisy at these sample
// sizes — that tends to hurt); we DETECT the clustering and warn the distiller so
// it doesn't over-credit the angle. 4 six-hour windows.
function timingConfound(top) {
  const hours = (top || []).map(p => p.posted_at ? new Date(p.posted_at).getUTCHours() : null).filter(h => h != null)
  if (hours.length < 4) return { confounded: false }
  const buckets = [0, 0, 0, 0]
  for (const h of hours) buckets[Math.floor(h / 6)]++
  const top1 = Math.max(...buckets) / hours.length
  const windows = ['00–06', '06–12', '12–18', '18–24']
  const peak = windows[buckets.indexOf(Math.max(...buckets))]
  return { confounded: top1 >= 0.6, concentration: +top1.toFixed(2), peak_window_utc: peak }
}
...
async function rewardArms(campaignId, signal, halfLifeWeeks) {
  ...
  for (const p of eligible) {
    ...
    await recordArmOutcome(campaignId, p.platform || 'x', 'angle_lens', p.campaign_arm.angle_lens, success, p._r.rate, { halfLifeWeeks })
    finalized.push(p.id)
  }
  if (finalized.length) {
...
// A learning is generic filler ("post more", "engage your audience", "create
// quality content") if it's short AND carries no concrete marker — a number, a
// quote, or a content lever (angle/hook/cta/format/…). Such platitudes get
// injected into every agent prompt and crowd out the real, specific learnings, so
// we drop them. Conservative: anything with a specificity signal always survives.
const GENERIC_RE = /^(post(ing)? (more|consistent|regular)|be (more )?(consistent|authentic|relatable)|engage (with )?(your )?audience|add(ing)? value|provide value|stay authentic|use (more )?hashtags|know your audience|create (good|quality|great|better|high.?quality|engaging) content|tell (a )?stor|show up|build (your )?(brand|community)|grow your (audience|following))/i
function isGenericLearning(text) {
  const t = squash(text).toLowerCase()
  if (t.split(' ').filter(Boolean).length < 6) return true
  const hasSpecific = /\d|%|"|“|\b(angle|hook|cta|call.to.action|format|carousel|thread|comparison|contrarian|question|story|behind.the.scenes|use.?case|price|pricing|demo|reply|replies|repost|caption|headline|emoji|hashtag|persona|link|video|photo|carousel)\b/i.test(t)
  if (hasSpecific) return false
  return GENERIC_RE.test(t) || /\b(more|consistent|authentic|valuable|engaging|quality|great)\b[\s\S]*\b(content|posts?|audience|engagement|following)\b/.test(t)
}
...
async function distill(signal, campaign) {
  ...
  // Quantified negativity flag — a real, sizeable negative signal (not a small-N
  // fluke) makes the cool-down learning MANDATORY rather than a vague suggestion.
  const negFlag = s.total >= 12 && (s.counts?.negative || 0) >= 3 && (s.neg_share || 0) >= 0.3
...
You get the campaign's best-performing posts (numbered T1, T2, …), its worst (B1, B2, …), and the audience's comment sentiment. Posts are scored by a REACH-NORMALIZED rate when reach is known (trust those most) — a high raw count on a low-reach post is NOT a winner. Find the PATTERNS: which ANGLE / hook / format / topic / persona drove real engagement, what fell flat, and what the audience signals they want or object to.

Write 2-6 learnings — FEWER, well-grounded learnings beat padding the list. Each must be:
- SPECIFIC and ACTIONABLE ("the side-by-side comparison angle out-pulls the generic tip on X", "answer the pricing question the audience keeps asking"), NEVER generic ("post more", "engage your audience", "create quality content", "be consistent"). Generic advice will be discarded.
- GROUNDED: cite the concrete evidence in the "evidence" field — the post label(s) it rests on (e.g. "T1, T3") plus the number/quote you saw ("rate 4.2% vs 0.9%"). If you cannot cite specific evidence, DO NOT emit the learning.
- A pattern you can see REPEATED, not a single lucky post — one outlier is not a learning.
...
GUARDRAIL: if sentiment skews negative, include a learning to cool the cadence or fix the message — NEVER a learning that exploits outrage for engagement.${negFlag ? '\nThis campaign HAS a significant negative-sentiment signal: you MUST emit one kind="audience" learning to slow down / fix the message, addressing the specific complaint above.' : ''}${signal.timing?.confounded ? `\nTIMING CAVEAT: the top posts cluster in the ${signal.timing.peak_window_utc} UTC window (${Math.round(signal.timing.concentration * 100)}% of winners) — some of that lift is posting-TIME luck, not the angle. Prefer time-insensitive patterns (angle/format/message/audience) and lower weights for anything that could just be timing.` : ''}`

  const labeled = (arr, pfx) => arr.map((p, i) => `${pfx}${i + 1}. ${line(p)}`).join('\n')
  const user = `CAMPAIGN: ${campaign.product || campaign.name}${campaign.pitch ? ` — ${campaign.pitch}` : ''}

TOP POSTS:
${labeled(signal.top, 'T')}

${signal.bottom.length ? `WEAKEST POSTS:\n${labeled(signal.bottom, 'B')}\n` : ''}
${sentLine}

Distill the durable campaign learnings now — grounded, specific, each with cited evidence.`

  const out = await generateJson({
    system: sys, user, maxTokens: 1500, toolName: 'emit_campaign_learnings',
    schema: {
      ...
      properties: {
        learnings: {
          ...
          items: {
            ...
            properties: {
              text: { type: 'string' },
              evidence: { type: 'string', description: 'the post label(s) + number/quote this rests on, e.g. "T1, T3 — rate 4.2% vs 0.9%"' },
  ...
  return (out.learnings || [])
    ...
    // Drop generic platitudes — they crowd out the specific learnings in agent prompts.
    .filter(l => l.text && !isGenericLearning(l.text))
    // Ungrounded learnings (no cited evidence) can't be trusted at high weight.
    .map(l => (l.evidence ? l : { ...l, weight: Math.min(l.weight, 2) }))
    .slice(0, 6)
}

export async function learnFromCampaign(campaignId, userId) {
  const { data: campaign } = await admin.from('agent_campaigns').select('id, user_id, name, product, pitch, recency_half_life_weeks').eq('id', campaignId).single()
  ...
  try {
    ...
    await rewardArms(campaignId, signal, campaign.recency_half_life_weeks).catch(() => {}) // bandit posteriors from settled trusted-reach posts (once each)
    // ROI guard: the bandit reward above is cheap, but distilling durable learnings
    // is an LLM call. Don't spend it on an inert campaign (too few posts or
    // essentially zero engagement) — there's nothing real to learn yet, and the
    // model would only emit generic filler. The bandit keeps adapting meanwhile.
    const totalEng = (signal.scored || []).reduce((n, p) => n + engScore(p), 0)
    if (signal.total < 5 || totalEng < 5) { await markLearned(campaignId, userId); return { skipped: 'thin signal', from_posts: signal.total } }
