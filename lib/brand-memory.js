...
// learnings, this-platform + cross-platform, strongest signal first. Auto-learned
// rows DECAY with age (read-time freshness, parity with campaign-memory) so a user
// who stops posting doesn't keep getting months-old, possibly-stale guidance;
// manual rows are ground truth and don't decay.
...
export async function getInsights(userId, platform = null, limit = 12, account = null) {
  const { data } = await admin.from('brand_memory')
    .select('kind, platform, text, weight, source, updated_at, x_connection_id, social_account_id')
  ...
  return (data || [])
    ...
    .map(r => {
      const decays = r.source === 'engagement' || r.source === 'trend'
    ...
    .filter(r => !((r.source === 'engagement' || r.source === 'trend') && r._eff < 0.6)) // drop stale low-weight auto rows
