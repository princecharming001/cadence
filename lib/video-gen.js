...
async function submit(path, params) {
  let r, netErr
  // Retry transient failures (network, 429, 5xx) so a momentary blip doesn't
  // permanently fail a valid job. Terminal codes (404/402/4xx) don't retry.
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    } catch (e) { netErr = e; r = null; continue }      // network — retry
    if (r.status === 429 || r.status >= 500) continue   // transient — retry
    break
  }
  if (!r) return { error: 'network: ' + String(netErr?.message || netErr || 'unreachable').slice(0, 120) }
...
export async function generateUgcVideo({ imageUrl, audioUrl, prompt, duration = 5 }) {
  ...
  const sub = await submit('/v1/speak/higgsfield', {
    ...
    // Speak only renders 5/10/15s — snap to the nearest bucket, don't collapse to 5.
    quality: 'mid', duration: [5, 10, 15].reduce((a, b) => Math.abs(b - (Number(duration) || 5)) < Math.abs(a - (Number(duration) || 5)) ? b : a, 5),
  })
  if (sub.error) return wrap(sub)
  return wrap(await poll(sub.reqId, 420000))
}
