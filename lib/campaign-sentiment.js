...
// Cheap pre-filter: obvious spam/bot/promo comments corrupt the sentiment signal
// (a viral post gets swarmed by "follow me" / link spam, which a classifier would
// score as positive/negative/question and weight equally with real audience
// reaction). Drop them BEFORE the LLM so they neither cost tokens nor pollute the
// rollup. Deliberately conservative — only clear junk; emoji-only reactions and
// short genuine comments pass through to the classifier.
const LINK_RE = /https?:\/\/|www\.[a-z0-9-]/i
const SPAM_RE = /\b(follow me|follow back|check my|check out my|dm me|sub(scribe)? to my|click (here|the link)|link in (bio|profile)|buy now|promo code|earn \$|make money|free followers|cash ?app|onlyfans|telegram|whats ?app me)\b/i
function isLikelySpam(text) {
  const t = String(text || '').trim()
  if (t.length < 2 || t.length > 1500) return true
  const letters = t.replace(/[^a-zA-Z]/g, '')
  if (letters.length >= 12 && letters === letters.toUpperCase()) return true // sustained ALL-CAPS shouting
  if (LINK_RE.test(t)) return true
...
async function gatherComments(userId, campaignId) {
  ...
  // The parent post text, so the classifier can read a comment IN CONTEXT — "great,
  // exactly what I needed" is genuine on a how-to post but sarcasm on a bug-report
  // post, and the aspect (hook/cta/price…) is only legible against what was posted.
  const excerptOf = p => String(p?.content || '').replace(/\s+/g, ' ').trim().slice(0, 140)

  const out = []
  const seen = new Set()
  const push = (cid, text, author, platform, post) => {
    const id = String(cid || '')
    const t = String(text || '').trim()
    if (!id || !t || seen.has(id)) return
    seen.add(id)
    out.push({ comment_id: id, comment_text: t.slice(0, 500), comment_author: String(author || '').slice(0, 80), platform, post_id: post?.id || null, post_excerpt: excerptOf(post) })
  }
    ...
    try {
      if ((p.platform === 'instagram' || p.platform === 'tiktok') && p.social_account_id && zernioEnabled()) {
        ...
        if (acct?.zernio_account_id) {
          ...
          for (const c of cs) push(commentId(c), commentText(c), commentAuthor(c), p.platform, p)
        }
      } else if (p.platform === 'linkedin' && unipileEnabled()) {
        if (uAcct === undefined) uAcct = await unipileAccountId(userId).catch(() => null)
        if (uAcct) { const cs = await listPostComments(uAcct, p.external_id, 30).catch(() => []); for (const c of cs) push(commentId(c), commentText(c), commentAuthor(c), 'linkedin', p) }
      }
    } catch {}
  }
  ...
  for (const r of replies || []) { const p = extToPost[String(r.post_id)]; push(r.comment_id, r.comment_text, r.comment_author, r.platform || p?.platform, p) }
...
async function classifyBatch(batch) {
  const sys = `You are a precise social-media comment analyst. Each line is a comment, optionally prefixed with the POST it replies to (in parentheses) — use that context to read the comment correctly. For EACH numbered comment, output:
...
- aspect: what the comment is REACTING TO — hook (the opening/attention grab), cta (the ask/link), topic (the subject), format (the style/medium), product (the thing promoted), price, or other. Judge it against the POST context when shown.
- is_sarcastic: true if the comment is ironic/sarcastic. SARCASM RULE: sarcasm inverts surface meaning and wrecks naive sentiment — use the POST context (e.g. praise on a complaint/bug post is likely sarcasm). When you suspect it, set is_sarcastic=true and LOWER confidence; never take a sarcastic comment at face value.
- confidence: 0-1.
Return one item per comment, echoing its index i.`
  const user = batch.map((c, i) => `[${i}]${c.post_excerpt ? ` (on post: "${c.post_excerpt}")` : ''} ${c.comment_text.slice(0, 240)}`).join('\n')
...
export async function analyzeCampaignSentiment(campaignId, userId) {
  ...
  // Drop obvious spam/bot junk BEFORE anything else, so it costs no tokens and the
  // real-audience comments get the full per-run budget.
  const clean = comments.filter(c => !isLikelySpam(c.comment_text))
  const droppedSpam = comments.length - clean.length
  ...
  const fresh = clean.filter(c => !seen.has(c.comment_id)).slice(0, 75) // bound per run
  if (!fresh.length) { await markSentimentRun(campaignId, userId); return { analyzed: 0, found: comments.length, dropped_spam: droppedSpam } }
  ...
  return { analyzed: rows.length, found: comments.length, dropped_spam: droppedSpam }
}
