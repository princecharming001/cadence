...
export async function analyzeCampaignSentiment(campaignId, userId) {
  ...
  if (!comments.length) return { analyzed: 0, found: 0 }
  ...
  if (!fresh.length) return { analyzed: 0, found: comments.length }
  ...
  for (let off = 0; off < fresh.length; off += 25) {
    ...
    batch.forEach((c, i) => {
      ...
      rows.push({
        ...
        sentiment: ['positive', 'neutral', 'negative', 'question'].includes(l.sentiment) ? l.sentiment : 'neutral',
        emotion: l.emotion || 'neutral', aspect: l.aspect || 'other',
        is_sarcastic: !!l.is_sarcastic, confidence: Math.min(Math.max(Number(l.confidence) || 0.5, 0), 1),
      })
    })
  }
