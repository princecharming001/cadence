...
export async function claimEngineRow(table, row) {
  ...
  // ±10% jitter so a fleet armed together doesn't stay phase-locked forever
  // (waking on the same cron tick is a coordination/bot signal).
  const nextMs = Date.now() + intervalH * 3600 * 1000 * (1 + (Math.random() * 0.2 - 0.1))
