...
// Batch-fetch public metrics for up to 100 tweets in ONE read — the cheapest
// way to close the analytics loop on published posts.
export async function lookupTweetMetrics(access_token, ids) {
  const batch = ids.slice(0, 100).join(',')
  const d = await xGet(access_token,
    `https://api.twitter.com/2/tweets?ids=${encodeURIComponent(batch)}&tweet.fields=public_metrics`)
  return Object.fromEntries((d.data || []).map(t => [t.id, t.public_metrics || {}]))
}
...
// Recent tweets matching a query (last 7 days). By default excludes retweets
// and replies so discovery engines target original posts — but callers hunting
// replies TO the user (the auto-reply inbox) MUST pass includeReplies: `to:user`
// only ever matches replies, so the blanket -is:reply filter zeroes it out.
export async function searchRecent(access_token, query, max = 10, { includeReplies = false } = {}) {
  ...
  const q = encodeURIComponent(`(${query}) -is:retweet${includeReplies ? '' : ' -is:reply'} lang:en`)
  const n = Math.min(Math.max(max, 10), 100) // API minimum is 10
  // reply_settings tells us up front whether a reply is even POSSIBLE — X rejects
  // a reply with a 403 when the author limited replies (following/mentioned/
  // verified/subscribers). Callers that reply (not just read) skip non-'everyone'.
  const d = await xGet(access_token,
    `https://api.twitter.com/2/tweets/search/recent?query=${q}&max_results=${n}&tweet.fields=author_id,text,public_metrics,created_at,reply_settings&expansions=author_id&user.fields=username`)
  ...
  return (d.data || []).map(t => ({
    ...
    reply_settings: t.reply_settings || 'everyone',
    url: `https://x.com/i/web/status/${t.id}`,
  }))
}

// Can THIS account reply to a tweet with these reply_settings? Only 'everyone' is
// open; the rest require following/mention/verification/subscription we can't
// guarantee, so a reply would 403. Treat unknown as open (don't over-filter).
export function canReplyTo(replySettings) {
  return !replySettings || replySettings === 'everyone'
}
