...
async function processZernioAgent(agent, persona, mission, counts) {
  ...
  if (plan.post && postBudget > 0) {
    ...
    if (isUgc && (platform === 'instagram' || platform === 'tiktok') && agent.soul_ref && videoProviderStatus() === 'ready') {
      ...
      if (script) {
        ...
        if (job?.id) {
          ...
          await insertAgentPost(agent, { content: caption, campaignId, isPromo: promo, campaignArm: promo ? { angle_lens: mission.lens } : null, status: 'rendering', videoJobId: job.id })
      ...
      // No script (FTC-blocked) or enqueue failed → fall through to a normal post.
    }
    ...
    if (platform === 'instagram' || platform === 'tiktok') {
      ...
      try {
        ...
        if (deck.imageUrls?.length) {
          if (await insertAgentPost(agent, { content: deck.caption, imageUrl: deck.imageUrls[0], imageUrls: deck.imageUrls, campaignId, isPromo: promo, campaignArm: promo ? { angle_lens: mission.lens } : null })) {
