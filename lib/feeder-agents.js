...
const ANGLE_LENSES = ['a personal story', 'a contrarian take', 'a concrete use-case', 'a question to your peers', 'a side-by-side comparison', 'a behind-the-scenes detail']
...
async function insertAgentPost(agent, { content, replyTo, targetText, targetUrl, imageUrl, imageUrls, campaignId, isPromo }) {
...
// Build the single chosen mission's prompt block for this cycle (with brand,
// per-agent lens, and a rationed link). Empty when no mission is eligible.
function buildMissionCtx(agent, persona, pick, brand) {
  ...
  return {
    eligible: true, campaign: c, lens: pick.lens,
    block: missionBlock(c, { intensity: inten, lens: pick.lens, link }) + brand,
  }
}
...
async function processZernioAgent(agent, persona, mission, counts) {
  ...
  if (plan.post && postBudget > 0) {
    ...
    if (platform === 'instagram' || platform === 'tiktok') {
      ...
      try {
        ...
        if (deck.imageUrls?.length) {
          if (await insertAgentPost(agent, { content: deck.caption, imageUrl: deck.imageUrls[0], imageUrls: deck.imageUrls, campaignId, isPromo: promo })) {
      ...
      } catch {} // deck failed → fall through to the caption + single image path
    }
    ...
    if (content) {
      ...
      if (await insertAgentPost(agent, { content, imageUrl, campaignId, isPromo: promo })) { outcomes.push(promo ? 'posted (mission)' : 'posted'); if (promo) promoCampaignId = campaignId }
    }
  }
  return { id: agent.id, outcomes, plan, promoCampaignId }
}
...
Decide your plan.`,
    ...
    if (plan.post && postBudget > 0) {
      ...
      if (content && await insertAgentPost(agent, { content, campaignId, isPromo: promo })) { outcomes.push(promo ? 'posted (mission)' : 'posted'); if (promo) promoCampaignId = campaignId }
    }
...
async function processAgent(agent) {
  try {
    ...
    const missionCtx = buildMissionCtx(agent, persona, pick, brand)
