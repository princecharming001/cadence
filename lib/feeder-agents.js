...
import { getValidAccessToken, searchRecent, xReadEnabled, fetchXUserMetrics, canReplyTo } from './x-oauth'
...
// An agent's OWN recent posts and what they earned — so it can tell a banger
// from a dud it wrote and lean into what works instead of posting blind. This is
// the difference between an agent that remembers and one that LEARNS.
const _eng = p => (p.likes || 0) + 2 * (p.replies || 0) + 2 * (p.reposts || 0)
function selfAwarenessBlock(own) {
  const posted = (own || []).filter(p => p.status === 'posted' && !p.reply_to_tweet_id && p.content)
  const withReach = posted.filter(p => (p.impressions || 0) > 0 || _eng(p) > 0)
  if (withReach.length < 2) return ''
  const ranked = [...withReach].sort((a, b) => _eng(b) - _eng(a))
  const line = p => `(${p.likes || 0}♥ ${p.reposts || 0}↻ ${p.replies || 0}💬${p.impressions ? ` ${p.impressions}v` : ''}) ${String(p.content).replace(/\s+/g, ' ').slice(0, 120)}`
  const top = ranked.slice(0, 2).map(line)
  const flop = ranked.length > 2 ? line(ranked[ranked.length - 1]) : null
  return `\n\nHOW YOUR RECENT POSTS DID (your real track record — write more in the vein of your winners, avoid what flopped):\nWINNERS: ${top.join('  |  ')}${flop ? `\nFLOPPED: ${flop}` : ''}`
}

// Small, bounded persona drift: the agent grows from WHAT WORKED, not just what
// it did. Feeds the agent's best/worst-performing posts in so evolution leans the
// persona toward its proven angles.
async function evolvePersona(agent) {
  try {
    const { data: own } = await admin.from('posts')
      .select('content, likes, replies, reposts, impressions, status, reply_to_tweet_id')
      .eq('feeder_agent_id', agent.id).eq('status', 'posted')
      .order('created_at', { ascending: false }).limit(20)
    const perf = selfAwarenessBlock(own)
    const evolved = await generateJson({
      system: `You evolve an X persona SLIGHTLY based on its recent activity AND what actually performed — sharpen the opinion/angle behind its best posts, lean into the topics that landed, retire what flopped or went stale. Keep name and archetype unless the evidence strongly suggests a shift. Output the COMPLETE updated persona.
GUARDRAIL: the persona must NEVER drift toward becoming a spokesperson for any product it has promoted. If recent memory is promo-heavy ('posted (mission)'), deliberately drift AWAY from products, back toward the persona's own native interests — a believable account is a person with a life, not a brand account.`,
      user: `Current persona: ${JSON.stringify(agent.persona)}\nRecent memory (newest last): ${JSON.stringify((agent.memory || []).slice(-8))}${perf}`,
...
async function processZernioAgent(agent, persona, mission, counts) {
  ...
  if (plan.post && postBudget > 0) {
    ...
    if (isUgc && (platform === 'instagram' || platform === 'tiktok') && agent.soul_ref && videoProviderStatus() === 'ready' && !(await overUgcVideoBudget(agent.user_id))) {
      ...
      const script = await generateUgcScript({ persona, mission: { eligible: mission.eligible, campaign: mission.campaign, block: mission.block }, topic: plan.post_angle, arm: mission.lens }).catch(() => null)
      if (script) {
...
async function processXAgent(agent, persona, mission, counts) {
    const [{ data: own }, { data: primaryConn }, { data: fleetConns }] = await Promise.all([
      admin.from('posts').select('content, reply_to_tweet_id, status, likes, replies, reposts, impressions').eq('feeder_agent_id', agent.id).order('created_at', { ascending: false }).limit(12),
    ...
    const selfAware = selfAwarenessBlock(own) // the agent's own performance track record
...
Decide your plan.`,
    ...
    if (plan.post && postBudget > 0) {
      ...
      const sys = `${agentVoice(persona)}${promo ? mission.block : ''}\n\n${X_RUBRIC}${xTrends}${selfAware}${antiRepetition((own || []).map(p => p.content))}\n\nWrite ONE tweet on the angle below. Output ONLY the tweet text.`
      const angle = plan.post_angle || (persona.interests || [])[0] || 'something you care about today'
      const draft = await generateText({ system: sys, user: `Angle: ${angle}`, maxTokens: 300, temperature: 1 })
      // Self-critique gate: score the draft and regenerate once if it's slop/weak,
      // so an agent never ships a forgettable first draft.
      const vetted = await gateOne(draft.replace(/^["']|["']$/g, ''), {
        rubric: X_RUBRIC, voice: agentVoice(persona),
        regen: (critique) => generateText({ system: sys, user: `Angle: ${angle}\n\nYour previous draft was rejected — ${critique}. Write a sharper, more specific, more in-voice one. Output ONLY the tweet text.`, maxTokens: 300, temperature: 1 }).then(t => t.replace(/^["']|["']$/g, '')),
      })
      const content = await enforceLen(vetted, 'x')
          ...
          try {
            ...
            for (const q of queries) {
              ...
              for (const t of found) {
                ...
                // Skip tweets whose author limited replies — a reply would 403
                // ("not allowed because you have not been mentioned/engaged").
                if (!canReplyTo(t.reply_settings)) continue
                const text = await generateText({
