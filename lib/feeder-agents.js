    ...
    try {
      if (a.x_connection_id) {
        ...
        const { data: acct } = await admin.from('social_accounts')
          .select('avatar, followers, platform, zernio_account_id').eq('id', a.social_account_id).single()
        if (!acct) continue
        const stats = { v: STATS_V, followers: acct.followers ?? null, avatar: acct.avatar || null, fetched_at: new Date().toISOString() }
        // Per-platform depth, each from the best real source available:
        //   TikTok    → video_count (+ followers fallback) via account insights
        //   Instagram → 30-day reach via account insights (no media count API)
        //   LinkedIn  → posts we scraped from their own profile (no public API)
        try {
          if (acct.platform === 'tiktok' && acct.zernio_account_id) {
...
async function insertAgentPost(agent, { content, replyTo, targetText, targetUrl, imageUrl, imageUrls }) {
...
async function processZernioAgent(agent, persona, mission) {
  ...
  if (plan.post && postBudget > 0) {
    ...
    // same pipeline as the studio, in the agent's voice — never a "slide 1:"
    // script pasted into a caption with a random image.
    if (platform === 'instagram' || platform === 'tiktok') {
      ...
      try {
        ...
        // Style is the agent's signature (stable per persona); format rotates.
        ...
        if (deck.imageUrls?.length) {
          if (await insertAgentPost(agent, { content: deck.caption, imageUrl: deck.imageUrls[0], imageUrls: deck.imageUrls })) {
            outcomes.push(plan.promo ? 'posted a carousel (mission)' : 'posted a carousel')
          }
