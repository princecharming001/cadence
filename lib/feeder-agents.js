...
export async function refreshAgentStats(userId) {
  const { data: agents } = await admin.from(TABLE)
    .select('id, x_connection_id, social_account_id, stats').eq('user_id', userId)
  const staleMs = Date.now() - 24 * 3600 * 1000
  const stale = (agents || []).filter(a => !(a.stats?.fetched_at && new Date(a.stats.fetched_at).getTime() > staleMs))
    ...
    try {
      if (a.x_connection_id) {
        ...
        const { data: acct } = await admin.from('social_accounts')
          .select('avatar, followers').eq('id', a.social_account_id).single()
        if (!acct) continue
        await admin.from(TABLE).update({
          stats: { followers: acct.followers ?? null, avatar: acct.avatar || null, fetched_at: new Date().toISOString() },
        }).eq('id', a.id)
      }
    } catch {}
  }
