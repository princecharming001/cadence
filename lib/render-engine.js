...
import { normalizeSeg, download, concatSegments, assembleSegments, canvasFor, reframe, ENCODE_ARGS, run } from './video-edit'
...
// ffmpeg 4.3+ ships xfade and 2.x ships zoompan, so any modern worker build has
// them (verified locally on ffmpeg 8.0.1). If a prod worker ever ships a minimal
// ffmpeg, flip these off and the normalizer downgrades transitions→cut / kenBurns→none.
const WORKER_FILTERS = { xfade: true, zoompan: true, acrossfade: true }
...
async function compositeScene(scene, src, job, dir, W, H, out, dur) {
  ...
  const t = String(dur)
  const ss = scene.trim_start != null && Number(scene.trim_start) > 0 ? ['-ss', String(Number(scene.trim_start))] : []
  const inArgs = ['-y']
  if (src.isImage) inArgs.push('-loop', '1', '-t', t, '-i', src.path)
  else inArgs.push(...ss, '-t', t, '-i', src.path)
  for (const im of imgInputs) inArgs.push('-loop', '1', '-t', t, '-i', im.path)

  // Background: reframe, or a slow ken-burns push on a still when motion=kenBurns.
  let bgChain
  if (scene.motion === 'kenBurns' && src.isImage) {
    // d=1 with a looped input (NOT d=frames — that multiplies every input frame).
    bgChain = `[0:v]scale=${W * 2}:${H * 2}:force_original_aspect_ratio=increase,crop=${W * 2}:${H * 2},zoompan=z='min(zoom+0.0011,1.30)':d=1:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=${W}x${H}:fps=30,setsar=1,format=yuv420p[bg]`
  } else {
    bgChain = `[0:v]${reframe(W, H)}[bg]`
  }
  const fc = [bgChain]
  ...
  await run('ffmpeg', [...inArgs, '-filter_complex', fc.join(';'), '-map', `[${last}]`, '-an', '-t', t, ...ENCODE_ARGS, out])
  return out
}
...
export async function renderEditPlan(rawPlan, job) {
  ...
  try {
    ...
    const segs = []
    const segMeta = []   // parallel to segs: { dur, transition, transitionDur } for assembly
    const skipped = []
    let pinned = false
    ...
    for (const scene of plan.scenes) {
      ...
      try {
        ...
        // a bare background reuses the proven, cheaper normalizeSeg (with trim/
        // ken-burns). Both emit ENCODE_ARGS-identical segments.
        if (Array.isArray(scene.elements) && scene.elements.length)
          await compositeScene(scene, src, job, dir, W, H, seg, dur)
        else
          await normalizeSeg(src.path, src.isImage, W, H, seg, dur, { ss: scene.trim_start, kenBurns: scene.motion === 'kenBurns' })
        if ((await stat(seg)).size > 0) { segs.push(seg); segMeta.push({ dur, transition: scene.transition, transitionDur: scene.transition_dur }) }
    ...
    if (segs.length < need) {
      ...
      if (fillSeg) for (let i = segs.length; i < need; i++) { segs.push(fillSeg); segMeta.push({ dur: 3, transition: 'cut', transitionDur: 0.3 }) }
    }
    ...
    // Cut boundaries → fast concat -c copy; any non-cut transition → xfade fold.
    const final = await assembleSegments(segs, segMeta.map(m => ({ name: m.transition, dur: m.transitionDur })), segMeta.map(m => m.dur), dir)
