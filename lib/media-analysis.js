...
// Big videos are NOT downloaded whole (a 6GB buffer would OOM the worker) — we
// point ffprobe/ffmpeg straight at the URL, which seeks via HTTP range requests
// and only pulls the few bytes each sampled frame needs. Small files still
// download locally (faster for many small seeks).
const STREAM_OVER_BYTES = 300 * 1024 * 1024
export async function analyzeVideo(asset) {
  const dir = await mkdtemp(path.join(tmpdir(), 'media-'))
  try {
    let src
    if ((Number(asset.size_bytes) || 0) > STREAM_OVER_BYTES) {
      src = asset.url // stream: ffmpeg reads over HTTP with range seeks, no full download
    } else {
      src = path.join(dir, 'in')
