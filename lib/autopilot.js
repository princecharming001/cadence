...
import { runSocialAutopilot } from './social-autopilot'

export const AUTOPILOT_PLATFORMS = ['x', 'linkedin', 'instagram', 'tiktok']

// Run one due autopilot row.
export async function runAutopilot(row) {
  // Instagram/TikTok are visual — route to the content engine (carousels / UGC /
  // clips from the content_plan). X/LinkedIn keep the text path below.
  if (row.platform === 'instagram' || row.platform === 'tiktok') return runSocialAutopilot(row)
  ...
  try {
    ...
    // Then honor the user's choice (set at onboarding, gated server-side):
    //   • auto_post → promote each draft to a queued post at the next smart slot,
    //                 spread across the day (true hands-off).
    //   • else      → leave them as drafts for the user to review & approve. We
    //                 never silently post in someone's name they didn't opt into.
    ...
    const made = drafts?.length || 0
    const detail = autoPost
      ? `Queued ${queued} post${queued === 1 ? '' : 's'}`
      : `Drafted ${made} post${made === 1 ? '' : 's'} for review`
    await admin.from('autopilot').update({ running: false, status_detail: detail }).eq('id', row.id)
    return { generated: made, queued, drafted: autoPost ? 0 : made }
  } catch (e) {
