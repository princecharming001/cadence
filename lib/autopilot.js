...
export async function runAutopilot(row) {
  ...
  try {
    ...
    await admin.from('autopilot').update({ running: false, status_detail: row.auto_post ? `Queued ${queued} post${queued === 1 ? '' : 's'}` : `Drafted ${drafts?.length || 0} for review` }).eq('id', row.id)
