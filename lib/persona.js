...
export async function analyzePersona(userId) {
  ...
  // Content pulled from their other connected accounts (X/IG/TikTok/LinkedIn).
  const { data: samples } = await admin.from('voice_samples').select('platform, text, metric').eq('user_id', userId).order('metric', { ascending: false }).limit(60)
  const sampleList = samples || []
  if (!posts.length && !sampleList.length) throw new Error('Nothing to learn from yet — add your LinkedIn, or pull content from a connected account (the “Learn my voice” button on each platform tab).')

  const corpus = [
    ...posts.map((p, i) => `[LinkedIn ${i + 1}] (${p.likes} likes) ${p.content}`),
    ...sampleList.map((s, i) => `[${s.platform} ${i + 1}]${s.metric ? ` (${s.metric})` : ''} ${s.text}`),
  ].join('\n\n')

  const json = await generateJson({
    system: `You are a brand-voice analyst. Given a person's social posts across platforms (LinkedIn, X, Instagram, TikTok), distill their ONE consistent writing voice and personality into a reusable profile for generating on-brand posts everywhere. Ground every field in the actual posts — quote-level specificity, not horoscope generalities.`,
    user: `Analyze this person's voice from their posts across platforms:\n\n${corpus.slice(0, 60000)}`,
    schema: {
      type: 'object',
      properties: {
        summary: { type: 'string', description: '2-3 sentences on who they are and how they write' },
        tone: { type: 'string', description: 'short tone descriptor' },
        topics: { type: 'array', items: { type: 'string' }, description: '5-8 themes they post about' },
        style_rules: { type: 'array', items: { type: 'string' }, description: '5-8 concrete do/don\'t rules capturing their style' },
        signature_moves: { type: 'array', items: { type: 'string' }, description: '3-5 recurring rhetorical patterns' },
        sample_hooks: { type: 'array', items: { type: 'string' }, description: '3-5 example opening lines in their voice' },
      },
      required: ['summary', 'tone', 'topics', 'style_rules'],
    },
    maxTokens: 1600, toolName: 'emit_voice_profile',
  })

  // Never clobber a good persona with a degenerate one: generateJson already
  // enforced the required fields, this is the belt to its suspenders.
  if (!json.tone?.trim() || !json.topics?.length || !json.style_rules?.length) {
    throw new Error('Voice analysis came back incomplete — kept your existing voice profile. Try again.')
  }
  ...
  const row = {
    ...
    source_post_count: posts.length + sampleList.length,
