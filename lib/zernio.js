...
// Unlink an account at Zernio so it's truly disconnected (not just forgotten
// locally). Best-effort: tolerate an unexpected API shape — the tombstone in the
// route guarantees it stays gone locally either way.
export async function disconnectAccount(zernioAccountId, userId) {
  if (!zernioAccountId) return false
  const pid = await ensureProfile(userId).catch(() => null)
  const attempts = [
    () => zfetch(`/accounts/${encodeURIComponent(zernioAccountId)}`, { method: 'DELETE' }),
    () => zfetch(`/accounts/${encodeURIComponent(zernioAccountId)}?profileId=${encodeURIComponent(pid || '')}`, { method: 'DELETE' }),
  ]
  for (const a of attempts) { try { await a(); return true } catch (e) { if (e.notConfigured) return false } }
  return false
}
...
export async function syncAccounts(userId) {
  ...
  // Accounts the user explicitly disconnected — never re-add them even if Zernio
  // still lists them (the unlink can lag or be unsupported).
  const { data: tomb } = await admin.from('social_disconnects').select('zernio_account_id').eq('user_id', userId)
  const tombstoned = new Set((tomb || []).map(r => r.zernio_account_id))
  const liveIds = new Set()
  for (const a of accounts) {
    const zid = a._id || a.id || a.accountId
    if (tombstoned.has(zid)) continue // disconnected → skip; any local row falls to stale-delete below
    liveIds.add(zid)
    // Real account-level numbers ride the account object: followersCount at the
    // top level; the platform's own post count hides in metadata.profileData
    // (Instagram mediaCount, TikTok videoCount). Audited live 2026-06-12.
    const extra = a.metadata?.profileData?.extraData || {}
    const postsCount = [extra.mediaCount, extra.videoCount, extra.postsCount].find(Number.isFinite)
    const row = {
      user_id: userId, zernio_account_id: zid, platform: a.platform,
      username: a.username || a.displayName || a.name || null,
      avatar: a.metadata?.profileData?.profilePicture || a.profilePicture || a.avatar || a.profileImageUrl || null,
      followers: Number.isFinite(a.followersCount) ? a.followersCount : null,
...
// Account-level insights (Meta-insights response shape; tolerant parser).
// instagram → reach/views/engagement; tiktok → follower/likes/video_count.
export async function accountInsights(platform, zernioAccountId, metrics) {
  const qs = new URLSearchParams({ accountId: zernioAccountId })
  if (metrics) qs.set('metrics', metrics)
  const d = await zfetch(`/analytics/${encodeURIComponent(platform)}/account-insights?${qs.toString()}`)
  const list = d.metrics || d.insights || d.data || (Array.isArray(d) ? d : [])
  const val = (name) => {
    const m = Array.isArray(list) ? list.find(x => (x.name || x.metric) === name) : null
    if (!m) return null
    if (m.total_value != null) return Number(m.total_value?.value ?? m.total_value) || 0
    const vs = m.values || []
    if (vs.length) return Number(vs[vs.length - 1]?.value) || 0
    return m.value != null ? Number(m.value) || 0 : null
  }
  return { val }
}

// Per-POST analytics → the metrics that feed the campaign learning loop for
// IG/TikTok (likes/comments/shares/reach). Tolerant of the response shape, like
// accountInsights. Endpoint/param names mirror the account-insights route; the
// exact post-analytics path is the one thing to re-verify against
// docs.zernio.com when ZERNIO_API_KEY is first configured.
export async function postInsights(platform, zernioAccountId, postId) {
  const qs = new URLSearchParams({ accountId: zernioAccountId, postId: String(postId) })
  const d = await zfetch(`/analytics/${encodeURIComponent(platform)}/post-insights?${qs.toString()}`)
  const list = d.metrics || d.insights || d.data || (Array.isArray(d) ? d : [])
  const val = (names) => {
    for (const name of names) {
      ...
      if (m) {
        ...
        const vs = m.values || []
        if (vs.length) return Number(vs[vs.length - 1]?.value) || 0
        if (m.value != null) return Number(m.value) || 0
      }
      if (d && d[name] != null) return Number(d[name]) || 0 // flat-object shape fallback
    }
    return 0
  }
  return {
    likes: val(['likes', 'like_count', 'like', 'reactions']),
    replies: val(['comments', 'comment_count', 'comments_count']),
    reposts: val(['shares', 'share_count', 'shares_count', 'saved', 'saves', 'save_count']),
    impressions: val(['reach', 'impressions', 'impression_count', 'plays', 'play_count', 'views', 'video_views', 'view_count']),
  }
}
