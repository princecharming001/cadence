...
export async function connectUrl(userId, platform) {
  const pid = await ensureProfile(userId)
  const d = await zfetch(`/connect/${encodeURIComponent(platform)}?profileId=${encodeURIComponent(pid)}`)
...
export async function syncAccounts(userId) {
  ...
  if (accounts.length) {
    await admin.from('social_accounts').insert(accounts.map(a => ({
      ...
      avatar: a.avatar || a.profileImageUrl || null,
    })))
  }
