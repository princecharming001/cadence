...
export async function createPost({ userId, accounts, content, mediaUrls = [], scheduledFor, timezone, title }) {
  ...
  const platforms = accounts.map(a => {
    ...
    if (a.platform === 'tiktok') item.customContent = shortTitle           // TikTok slideshow title (<=90)
