...
import { voiceBlock, feedbackBlock, antiRepetition, X_RUBRIC, LINKEDIN_RUBRIC, enforceLen } from './prompts'
...
const SURFACE = {
  ...
  linkedin: { rubric: LINKEDIN_RUBRIC, register: 'longform', brief: 'LinkedIn posts, 400-900 characters each.' },
}
...
export async function generateSuggestions(userId, platform, n = 3) {
  ...
  const [{ persona, fb, recent, winners }, inspo] = await Promise.all([
    getVoice(userId),
...
${surface.rubric}${feedbackBlock(fb)}${winnersBlock(winners)}${inspoBlock}${antiRepetition(recent)}
