...
export async function generateSuggestions(userId, platform, n = 3) {
  ...
  const [{ persona, fb, recent, winners }, inspo, trending] = await Promise.all([
    getVoice(userId),
...
${surface.rubric}${feedbackBlock(fb)}${winnersBlock(winners)}${inspoBlock}${trending}${antiRepetition(recent)}
