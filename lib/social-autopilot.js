...
async function produce(row, plan, format, mem, autoPost, after) {
  ...
  if (format === 'ugc_face') {
    if (!plan.face_photo_url || videoProviderStatus() !== 'ready') return null
...
export async function runSocialAutopilot(row) {
  ...
  try {
    ...
    // Resolve the account for this platform at run time (autopilot is keyed by
    // user+platform; the poster also resolves it, but pinning it on the row lets a
    // carousel/video target the right account).
    if (!row.social_account_id) {
      const { data: acct } = await admin.from('social_accounts').select('id').eq('user_id', row.user_id).eq('platform', row.platform).limit(1).single()
