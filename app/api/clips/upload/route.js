...
export async function POST(req) {
  ...
  if (!/^video\//.test(file.type)) return Response.json({ error: 'Upload a video file (mp4/mov).' }, { status: 400 })
  if (file.size > 520 * 1024 * 1024) return Response.json({ error: 'Video is over 500MB — trim it down first.' }, { status: 400 })

  const stamp = Date.now().toString(36)
  const ext = (file.name?.split('.').pop() || 'mp4').toLowerCase().replace(/[^a-z0-9]/g, '') || 'mp4'
  const path = `${user.id}/sources/${stamp}.${ext}`
